...
class DocuParam extends Parameter {
  ...
  /** Holds if this parameter has a non-trivial `@java_param` tag. */
  predicate hasAcceptableParamTag() {
    exists(ParamTag t |
      t = this.getCallable().getDoc().getJavadoc().getATag("@java_param") and
...
class DocuReturn extends DocuCallable {
  ...
  /** Holds if this callable's `Javadoc` has a non-trivial `@java_return` tag. */
  predicate hasAcceptableReturnTag() {
    acceptableTag(this.getDoc().getJavadoc().getATag("@java_return"))
  }
}
...
class DocuThrows extends DocuCallable {
  ...
  /**
   * Holds if this callable has a non-trivial Javadoc `@java_throws` or `@java_exception` tag
  ...
  predicate hasAcceptableThrowsTag(Exception e) {
    exists(Javadoc j |
      ...
      exists(JavadocTag t |
        (t = j.getATag("@java_throws") or t = j.getATag("@java_exception")) and
