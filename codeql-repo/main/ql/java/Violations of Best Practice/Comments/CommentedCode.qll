 ...
 *
 * Matches comment lines ending with `{`, `}` or `;` that do not start with `>` or `@java_`, but first filters out:
 *
 * - Lines containing `//`
 * - Substrings between `{@java_` and `}` (including the brackets themselves)
