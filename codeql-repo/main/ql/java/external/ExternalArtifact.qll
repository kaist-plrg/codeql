...
class ExternalData extends @java_externalDataElement {
  string getDataPath() { java_externalData(this, result, _, _) }

  string getQueryPath() { result = getDataPath().regexpReplaceAll("\\.[^.]*$", ".ql") }

  int getNumFields() { result = 1 + max(int i | java_externalData(this, _, i, _) | i) }

  string getField(int index) { java_externalData(this, _, index, result) }
