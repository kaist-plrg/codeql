...
class Element extends @java_element, Top {
...
private predicate hasChildElement(Element parent, Element e) {
  java_cupackage(e, parent)
  or
  java_enclInReftype(e, parent)
  or
  not java_enclInReftype(e, _) and
  e.(Class).getCompilationUnit() = parent
  or
  not java_enclInReftype(e, _) and
  e.(Interface).getCompilationUnit() = parent
  or
  java_methods(e, _, _, _, parent, _)
  or
  java_constrs(e, _, _, _, parent, _)
  or
  java_params(e, _, _, parent, _)
  or
  java_fields(e, _, _, parent, _)
  or
  java_typeVars(e, _, _, _, parent)
}
