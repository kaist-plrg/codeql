...
predicate hasSubtype(RefType t, Type sub) {
  // Direct subtype.
  java_extendsReftype(sub, t) and t != sub
  or
  java_implInterface(sub, t)
  or
...
predicate declaresMember(Type t, @java_member m) {
  java_methods(m, _, _, _, t, _)
  or
  java_constrs(m, _, _, _, t, _)
  or
  java_fields(m, _, _, t, _)
  or
  java_enclInReftype(m, t) and
  // Since the type `@java_member` in the dbscheme includes all `@java_reftype`s,
 ...
 */
class Type extends Element, @java_type {
  /**
...
class Array extends RefType, @java_array {
   ...
   */
  Type getElementType() { java_arrays(this, _, result, _, _) }
   ...
   */
  int getDimension() { java_arrays(this, _, _, result, _) }
 ...
 */
class RefType extends Type, Annotatable, Modifiable, @java_reftype {
  /** Gets the package in which this type is declared. */
  Package getPackage() {
    java_classes(this, _, result, _) or
    java_interfaces(this, _, result, _)
  }

  /** Gets the type in which this reference type is enclosed, if any. */
  RefType getEnclosingType() { java_enclInReftype(this, result) }
  ...
  predicate extendsOrImplements(RefType t) {
    java_extendsReftype(this, t) or
    java_implInterface(this, t) or
  ...
  private predicate hasNonInterfaceMethod(Method m, RefType declaringType, boolean hidden) {
    ...
    exists(RefType sup, boolean h1, boolean h2 |
      ...
      exists(string signature |
        java_methods(m, _, signature, _, _, _) and not java_methods(_, _, signature, _, this, _)
      ) and
      m.isInheritable()
    )
  }
  ...
  private predicate cannotInheritInterfaceMethod(string signature) {
    java_methods(_, _, signature, _, this, _)
    or
    exists(Method m | this.hasNonInterfaceMethod(m, _, false) and java_methods(m, _, signature, _, _, _))
  }
  ...
  private predicate interfaceMethodCandidateWithSignature(
    ...
    java_methods(m, _, signature, _, _, _)
    or
    exists(RefType sup |
...
/** A class declaration. */
class Class extends RefType, @java_class {
  /** Holds if this class is an anonymous class. */
  predicate isAnonymous() { java_isAnonymClass(this, _) }

  /** Holds if this class is a local class. */
  predicate isLocal() { java_isLocalClass(this, _) }

  override RefType getSourceDeclaration() { java_classes(this, _, _, result) }
...
class Record extends Class {
  Record() { java_isRecord(this) }
}

/** An intersection type. */
class IntersectionType extends RefType, @java_class {
  IntersectionType() {
    exists(string shortname |
      java_classes(this, shortname, _, _) and
      shortname.matches("% & ...")
    )
  }

  private RefType superType() { java_extendsReftype(this, result) }

  private RefType superInterface() { java_implInterface(this, result) }
  ...
  RefType getFirstBound() { java_extendsReftype(this, result) }
}
...
class AnonymousClass extends NestedClass {
  ...
  ClassInstanceExpr getClassInstanceExpr() { java_isAnonymClass(this, result) }
...
class LocalClass extends NestedClass {
  ...
  LocalClassDeclStmt getLocalClassDeclStmt() { java_isLocalClass(this, result) }
...
class TopLevelType extends RefType {
  TopLevelType() {
    not java_enclInReftype(this, _) and
...
class NestedType extends RefType {
  NestedType() { java_enclInReftype(this, _) }

  /** Gets the type enclosing this nested type. */
  override RefType getEnclosingType() { java_enclInReftype(this, result) }
...
/** An interface. */
class Interface extends RefType, @java_interface {
  override RefType getSourceDeclaration() { java_interfaces(this, _, _, result) }
 ...
 * and `double`.
 */
class PrimitiveType extends Type, @java_primitive {
...
class NullType extends Type, @java_primitive {
...
/** The `void` type. */
class VoidType extends Type, @java_primitive {
...
class EnumType extends Class {
  EnumType() { java_isEnumType(this) }

  /** Gets the enum constant with the specified name. */
  EnumConstant getEnumConstant(string name) {
    java_fields(result, _, _, this, _) and result.hasName(name)
  }

  /** Gets an enum constant declared in this enum type. */
  EnumConstant getAnEnumConstant() { java_fields(result, _, _, this, _) }
...
class EnumConstant extends Field {
  EnumConstant() { java_isEnumConst(this) }
