  ...
  SpringBasePackage() {
    exists(string basePackages |
      ...
      exists(SpringComponent c, Annotation componentScan |
        ...
        // For a `@java_ComponentScan` annotation to take effect, the configuration class must already be
...
class SpringComponent extends RefType {
  ...
  /**
   * Gets the `@java_Component` or equivalent annotation.
   */
  ...
  predicate isLive() {
    ...
    // particular base packages. Base packages can be defined either using the `@java_ComponentScan`
    // annotation, on an `@java_Configuration` class, or in an XML configuration file. We can therefore
