...
/**
 * Holds if the annotatable has the JaxRs `@java_Path` annotation.
 */
...
/**
 * A method which is annotated with one or more JaxRS resource type annotations e.g. `@java_GET`, `@java_POST` etc.
 */
...
/**
 * A JaxRs resource class, annotated with `@java_Path` or referred to from a sub-resource locator on
...
class JaxRsResourceClass extends Class {
  JaxRsResourceClass() {
    // A root resource class has a @java_Path annotation on the class.
   ...
   *
   * Resource methods may be executed in response to web requests which match the `@java_Path`
  ...
  /**
   * Gets a "sub-resource locator" on this resource class, which is a method annotated with `@java_Path`,
   * but is not a resource method e.g. it is not annotated with `@java_GET` etc.
   */
