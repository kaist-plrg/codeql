...
class SpringControllerAnnotation extends AnnotationType {
  SpringControllerAnnotation() {
    // `@java_Controller` used directly as an annotation.
    hasQualifiedName("org.springframework.stereotype", "Controller")
    or
    // `@java_Controller` can be used as a meta-annotation on other annotation types.
 ...
 *
 * Rest controllers are the same as controllers, but imply the `@java_ResponseBody` annotation.
 */
...
class SpringModelAttributeMethod extends SpringControllerMethod {
  SpringModelAttributeMethod() {
    // Any method that declares the @java_ModelAttribute annotation, or overrides a method that declares
    // the annotation. We have to do this explicit check because the @java_ModelAttribute annotation is
    // not declared with @java_Inherited.
...
class SpringInitBinderMethod extends SpringControllerMethod {
  SpringInitBinderMethod() {
    // Any method that declares the @java_InitBinder annotation, or overrides a method that declares
    // the annotation. We have to do this explicit check because the @java_InitBinder annotation is
    // not declared with @java_Inherited.
    exists(Method superMethod |
      this.overrides*(superMethod) and
...
class SpringRequestMappingAnnotationType extends AnnotationType {
  SpringRequestMappingAnnotationType() {
    // `@java_RequestMapping` used directly as an annotation.
    hasQualifiedName("org.springframework.web.bind.annotation", "RequestMapping")
    or
    // `@java_RequestMapping` can be used as a meta-annotation on other annotation types, e.g. GetMapping, PostMapping etc.
...
class SpringResponseBodyAnnotationType extends AnnotationType {
  SpringResponseBodyAnnotationType() {
    // `@java_ResponseBody` used directly as an annotation.
...
class SpringRequestMappingMethod extends SpringControllerMethod {
  ...
  SpringRequestMappingMethod() {
    // Any method that declares the @java_RequestMapping annotation, or overrides a method that declares
    // the annotation. We have to do this explicit check because the @java_RequestMapping annotation is
    // not declared with @java_Inherited.
    exists(Method superMethod |
      this.overrides*(superMethod) and
  ...
  /** Gets the "produces" @java_RequestMapping annotation value, if present. */
  string getProduces() {
    result =
      requestMappingAnnotation.getValue("produces").(CompileTimeConstantExpr).getStringValue()
  }

  /** Holds if this is considered an `@java_ResponseBody` method. */
...
class SpringRequestMappingParameter extends Parameter {
  ...
  private predicate isExplicitlyTaintedInput() {
    ...
    // HttpEntity is like @java_RequestBody, but with a wrapper including the headers
  ...
  private predicate isImplicitRequestParam() {
    // Any parameter which is not explicitly handled, is consider to be an `@java_RequestParam`, if
  ...
  private predicate isImplicitModelAttribute() {
    // Any parameter which is not explicitly handled, is consider to be an `@java_ModelAttribute`, if
  ...
  /** Holds if this is an explicit or implicit `@java_ModelAttribute` parameter. */
  ...
  predicate isTaintedInput() {
    ...
    or
    // Any parameter which is not explicitly identified, is consider to be an `@java_RequestParam`, if
    // it is a simple bean property) or a @java_ModelAttribute if not
 ...
 *
 * This includes types directly referred to as either `@java_ModelAttribute` or `@java_RequestBody` parameters,
