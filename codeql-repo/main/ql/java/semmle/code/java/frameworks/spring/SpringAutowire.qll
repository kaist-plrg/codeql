...
/**
 * Holds if an injection annotation, such as `@java_Autowired` or `@java_Inject`, is present on this particular
 * annotatable element.
 */
 ...
 *
 * Confusingly, this is a different form of autowiring to the `@java_Autowired` annotation.
 */
...
/**
 * A callable that is annotated with `@java_Autowired`.
 *
...
class SpringBeanAutowiredCallable extends Callable {
   ...
   * considering any `@java_Qualifier` annotations, and falling back to autowiring by type.
   */
...
/**
 * A field that is annotated with `@java_Autowired`.
 */
class SpringBeanAutowiredField extends Field {
  ...
  /**
   * Gets the `SpringBean`, if any, that will be injected for this field, considering any `@java_Qualifier`
  ...
  SpringBean getInjectedBean() {
    ...
    then
      // Resolved by `@java_Qualifier("qualifier")` specified on the field
      result = getQualifier().getSpringBean()
    else
   ...
   * `@java_Qualifier` annotations, and falling back to autowiring by type.
   */
...
/**
 * A `@java_Resource` annotation on a field or method that specifies that the field or method should be
