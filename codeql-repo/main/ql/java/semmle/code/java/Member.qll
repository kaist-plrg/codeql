 ...
 */
class Member extends Element, Annotatable, Modifiable, @java_member {
...
class Callable extends StmtParent, Member, @java_callable {
  /**
   * Gets the declared return type of this callable (`void` for
   * constructors).
   */
  Type getReturnType() {
    java_constrs(this, _, _, result, _, _) or
    java_methods(this, _, _, result, _, _)
  }
  ...
  Parameter getParameter(int n) { java_params(result, _, n, this, _) }

  /** Gets the type of the formal parameter at the specified (zero-based) position. */
  Type getParameterType(int n) { java_params(_, result, n, this, _) }
  ...
  Exception getAnException() { java_exceptions(result, _, this) }
   ...
   */
  string getSignature() {
    java_constrs(this, _, result, _, _, _) or
    java_methods(this, _, result, _, _, _)
  }
}
...
class Method extends Callable, @java_method {
  ...
  override string getSignature() { java_methods(this, _, result, _, _, _) }
  ...
  override SrcMethod getSourceDeclaration() { java_methods(this, _, _, _, _, result) }
...
class SrcMethod extends Method {
  SrcMethod() { java_methods(_, _, _, _, _, this) }
...
class Constructor extends Callable, @java_constructor {
  /** Holds if this is a default constructor, not explicitly declared in source code. */
  predicate isDefaultConstructor() { java_isDefConstr(this) }

  override Constructor getSourceDeclaration() { java_constrs(this, _, _, _, _, result) }

  override string getSignature() { java_constrs(this, _, result, _, _, _) }
...
class FieldDeclaration extends ExprParent, @java_fielddecl, Annotatable {
  ...
  Field getAField() { java_fieldDeclaredIn(result, this, _) }

  /** Gets the field declared at the specified (zero-based) position in this declaration */
  Field getField(int idx) { java_fieldDeclaredIn(result, this, idx) }

  /** Gets the number of fields declared in this declaration. */
  int getNumField() { result = max(int idx | java_fieldDeclaredIn(_, this, idx) | idx) + 1 }
...
class Field extends Member, ExprParent, @java_field, Variable {
  /** Gets the declared type of this field. */
  override Type getType() { java_fields(this, _, result, _, _) }

  /** Gets the type in which this field is declared. */
  override RefType getDeclaringType() { java_fields(this, _, _, result, _) }
   ...
   */
  Field getSourceDeclaration() { java_fields(this, _, _, _, result) }
