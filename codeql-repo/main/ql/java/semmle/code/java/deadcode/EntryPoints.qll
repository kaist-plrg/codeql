...
/**
 * An enum annotated with `@java_XmlEnum` can be used by JAXB when constructing objects reflectively based
 * on the contents of XML files. Unlike classes, these are never referred to from the `@java_XmlRegistry`
...
/**
 * A type annotated with `@java_XmlType`, indicating that this class is used when marshalling or
...
/**
 * A JAX RS resource class. `@java_GET` and `@java_POST` annotated methods are reflectively called by the container. The
