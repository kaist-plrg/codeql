...
class Annotation extends @java_annotation, Expr {
...
class DeclAnnotation extends @java_declannotation, Annotation { }

/** An `Annotation` that applies to a type. */
class TypeAnnotation extends @java_typeannotation, Annotation { }

/**
 * There may be duplicate entries in java_annotValue(...) - one entry for
...
class Annotatable extends Element {
  ...
  /**
   * Holds if this or any enclosing `Annotatable` has a `@java_SuppressWarnings("<category>")`
...
class AnnotationType extends Interface {
  AnnotationType() { java_isAnnotType(this) }

  /** Gets the annotation element with the specified `name`. */
  AnnotationElement getAnnotationElement(string name) {
    java_methods(result, _, _, _, this, _) and result.hasName(name)
  }

  /** Gets an annotation element that is a member of this annotation type. */
  AnnotationElement getAnAnnotationElement() { java_methods(result, _, _, _, this, _) }

  /** Holds if this annotation type is annotated with the meta-annotation `@java_Inherited`. */
...
class AnnotationElement extends Member {
  AnnotationElement() { java_isAnnotElem(this) }

  /** Gets the type of this annotation element. */
  Type getType() { java_methods(this, _, _, result, _, _) }
}
