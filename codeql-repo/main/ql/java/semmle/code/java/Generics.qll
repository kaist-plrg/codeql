...
class GenericType extends RefType {
  GenericType() { java_typeVars(_, _, _, _, this) }
   ...
   */
  TypeVariable getTypeParameter(int i) { java_typeVars(result, _, i, _, this) }
 ...
 */
abstract class BoundedType extends RefType, @java_boundedtype {
 ...
 */
class TypeVariable extends BoundedType, @java_typevariable {
  /** Gets the generic type that is parameterized by this type parameter, if any. */
  RefType getGenericType() { java_typeVars(this, _, _, _, result) }

  /** Gets the generic callable that is parameterized by this type parameter, if any. */
  GenericCallable getGenericCallable() { java_typeVars(this, _, _, _, result) }
 ...
 */
class Wildcard extends BoundedType, @java_wildcard {
  /**
   ...
   * has an upper bound.
   */
  predicate hasUpperBound() { java_wildcards(this, _, 1) }

  /** Holds if this wildcard has a lower bound. */
  predicate hasLowerBound() { java_wildcards(this, _, 2) }
 ...
 */
class TypeBound extends @java_typebound {
  /**
   ...
   */
  BoundedType getBoundedType() { java_typeBounds(this, _, _, result) }
   ...
   */
  RefType getType() { java_typeBounds(this, result, _, _) }
   ...
   */
  int getPosition() { java_typeBounds(this, _, result, _) }
...
class ParameterizedType extends RefType {
  ParameterizedType() {
    java_typeArgs(_, _, this) or
    java_typeVars(_, _, _, _, this)
  }
   ...
   */
  override RefType getErasure() { java_erasure(this, result) or this.(GenericType) = result }
  ...
  RefType getATypeArgument() {
    java_typeArgs(result, _, this) or
    java_typeVars(result, _, _, _, this)
  }

  /** Gets the type argument of this parameterized type at the specified position. */
  RefType getTypeArgument(int pos) {
    java_typeArgs(result, pos, this) or
    java_typeVars(result, _, pos, _, this)
  }
  ...
  int getNumberOfTypeArguments() {
    result =
      count(int pos |
        java_typeArgs(_, pos, this) or
        java_typeVars(_, _, pos, _, this)
      )
  }

  /** Holds if this type originates from source code. */
  override predicate fromSource() { java_typeVars(_, _, _, _, this) and RefType.super.fromSource() }
}
...
class RawType extends RefType {
   ...
   */
  override RefType getErasure() { java_erasure(this, result) }
...
class GenericCallable extends Callable {
  GenericCallable() {
    exists(Callable srcDecl |
      java_methods(this, _, _, _, _, srcDecl) or java_constrs(this, _, _, _, _, srcDecl)
    |
      java_typeVars(_, _, _, _, srcDecl)
    )
  }
   ...
   */
  TypeVariable getTypeParameter(int i) { java_typeVars(result, _, i, _, this.getSourceDeclaration()) }
