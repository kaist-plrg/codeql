...
class Modifier extends Element, @java_modifier {
  /** Gets the element to which this modifier applies. */
  Element getElement() { java_hasModifier(result, this) }
}
...
abstract class Modifiable extends Element {
   ...
   * does not.
   */
  predicate hasModifier(string m) { java_modifiers(getAModifier(), m) }

  /** Holds if this element has no modifier. */
  predicate hasNoModifier() { not java_hasModifier(this, _) }
