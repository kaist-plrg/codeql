...
private predicate smap(File inputFile, int inLine, File outputFile, int outLineStart, int outLineEnd) {
  exists(
    ...
    int n
  |
    java_smap_header(outputFile, _, defaultStratum) and
    java_smap_files(outputFile, defaultStratum, inputFileNum, _, inputFile) and
    java_smap_lines(outputFile, defaultStratum, inputFileNum, inStart, inCount, outStart, outIncr) and
...
predicate hasSmapLocationInfo(
  ...
  exists(File inputFile, File outputFile |
    ...
    java_locations_default(_, outputFile, osl, _, oel, _) and
