...
class Diagnostic extends Locatable, @cpp_diagnostic {
  /**
   ...
   */
  int getSeverity() { cpp_diagnostics(underlyingElement(this), result, _, _, _, _) }

  /** Gets the error code for this compiler message. */
  string getTag() { cpp_diagnostics(underlyingElement(this), _, result, _, _, _) }
   ...
   * diagnostic.
   */
  string getMessage() { cpp_diagnostics(underlyingElement(this), _, _, result, _, _) }

  /**
   * Gets the full error message text associated with this compiler
   * diagnostic.
   */
  string getFullMessage() { cpp_diagnostics(underlyingElement(this), _, _, _, result, _) }

  /** Gets the source location corresponding to the compiler message. */
  override Location getLocation() { cpp_diagnostics(underlyingElement(this), _, _, _, _, result) }
