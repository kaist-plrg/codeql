 ...
 * each `@cpp_compilation`, for use in the `toString` method of `Compilation`.
 * These integers are not stable across trap imports, but stable across
 * runs with the same database.
 */

private predicate id(@cpp_compilation x, @cpp_compilation y) { x = y }

private predicate idOf(@cpp_compilation x, int y) = equivalenceRelation(id/2)(x, y)
 ...
 * but not for step 1.
 */
class Compilation extends @cpp_compilation {
  ...
  File getFileCompiled(int i) { cpp_compilation_compiling_files(this, i, unresolveElement(result)) }

  /**
   * Gets the amount of CPU time spent processing file number `i` in the C++
   * front-end.
   */
  float getFrontendCpuSeconds(int i) { cpp_compilation_time(this, i, 1, result) }

  /**
   * Gets the amount of elapsed time while processing file number `i` in the
   * C++ front-end.
   */
  float getFrontendElapsedSeconds(int i) { cpp_compilation_time(this, i, 2, result) }

  /**
   * Gets the amount of CPU time spent processing file number `i` in the
   * extractor.
   */
  float getExtractorCpuSeconds(int i) { cpp_compilation_time(this, i, 3, result) }

  /**
   * Gets the amount of elapsed time while processing file number `i` in the
   * extractor.
   */
  float getExtractorElapsedSeconds(int i) { cpp_compilation_time(this, i, 4, result) }
   ...
   */
  predicate normalTermination() { cpp_compilation_finished(this, _, _) }
}
