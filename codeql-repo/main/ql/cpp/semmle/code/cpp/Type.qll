 ...
 */
class Type extends Locatable, @cpp_type {
  ...
  Specifier getASpecifier() {
    cpp_typespecifiers(underlyingElement(this), unresolveElement(result))
    or
   ...
   */
  Attribute getAnAttribute() { cpp_typeattributes(underlyingElement(this), unresolveElement(result)) }
   ...
   * to this `@cpp_type` in the database but arise through type aliases such as
   ...
   */
  Type getUnspecifiedType() { cpp_unspecifiedtype(underlyingElement(this), unresolveElement(result)) }
   ...
   */
  int getSize() {
    cpp_builtintypes(underlyingElement(this), _, _, result, _, _) or
    cpp_pointerishsize(underlyingElement(this), result, _) or
    cpp_usertypesize(underlyingElement(this), result, _)
  }
   ...
   */
  int getAlignment() {
    cpp_builtintypes(underlyingElement(this), _, _, _, _, result) or
    cpp_pointerishsize(underlyingElement(this), _, result) or
    cpp_usertypesize(underlyingElement(this), _, result)
  }
 ...
 * ```
 */
class BuiltInType extends Type, @cpp_builtintype {
  override string toString() { result = this.getName() }

  override string getName() { cpp_builtintypes(underlyingElement(this), result, _, _, _, _) }
...
class ErroneousType extends BuiltInType {
  ErroneousType() { cpp_builtintypes(underlyingElement(this), _, 1, _, _, _) }
...
class UnknownType extends BuiltInType {
  UnknownType() { cpp_builtintypes(underlyingElement(this), _, 2, _, _, _) }

  override string getAPrimaryQlClass() { result = "UnknownType" }
}

private predicate isArithmeticType(@cpp_builtintype type, int kind) {
  cpp_builtintypes(type, _, kind, _, _, _) and
  kind >= 4 and
...
private predicate isIntegralType(@cpp_builtintype type, int kind) {
...
class IntegralOrEnumType extends Type {
  IntegralOrEnumType() {
    ...
    // Enum type
    (
      cpp_usertypes(underlyingElement(this), _, 4) or
      cpp_usertypes(underlyingElement(this), _, 13)
    )
  }
}
...
class IntegralType extends ArithmeticType, IntegralOrEnumType {
  ...
  predicate isSigned() { cpp_builtintypes(underlyingElement(this), _, _, _, -1, _) }

  /** Holds if this integral type is unsigned. */
  predicate isUnsigned() { cpp_builtintypes(underlyingElement(this), _, _, _, 1, _) }

  /** Holds if this integral type is explicitly signed. */
  predicate isExplicitlySigned() {
    cpp_builtintypes(underlyingElement(this), _, 7, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 10, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 13, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 16, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 19, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 37, _, _, _)
  }

  /** Holds if this integral type is explicitly unsigned. */
  predicate isExplicitlyUnsigned() {
    cpp_builtintypes(underlyingElement(this), _, 6, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 9, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 12, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 15, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 18, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 36, _, _, _)
  }

  /** Holds if this integral type is implicitly signed. */
  predicate isImplicitlySigned() {
    cpp_builtintypes(underlyingElement(this), _, 5, _, -1, _) or
    cpp_builtintypes(underlyingElement(this), _, 8, _, -1, _) or
    cpp_builtintypes(underlyingElement(this), _, 11, _, -1, _) or
    cpp_builtintypes(underlyingElement(this), _, 14, _, -1, _) or
    cpp_builtintypes(underlyingElement(this), _, 17, _, -1, _) or
    cpp_builtintypes(underlyingElement(this), _, 35, _, -1, _)
  }
  ...
  IntegralType getUnsigned() {
    exists(int unsignedKind |
      ...
      cpp_builtintypes(unresolveElement(result), _, unsignedKind, _, _, _)
    )
  }
  ...
  IntegralType getCanonicalArithmeticType() {
    exists(int canonicalKind |
      ...
      cpp_builtintypes(unresolveElement(result), _, canonicalKind, _, _, _)
    )
  }
}
...
class BoolType extends IntegralType {
  BoolType() { cpp_builtintypes(underlyingElement(this), _, 4, _, _, _) }
...
class CharType extends IntegralType {
  CharType() { cpp_builtintypes(underlyingElement(this), _, [5, 6, 7], _, _, _) }
}
...
class PlainCharType extends CharType {
  PlainCharType() { cpp_builtintypes(underlyingElement(this), _, 5, _, _, _) }
...
class UnsignedCharType extends CharType {
  UnsignedCharType() { cpp_builtintypes(underlyingElement(this), _, 6, _, _, _) }
...
class SignedCharType extends CharType {
  SignedCharType() { cpp_builtintypes(underlyingElement(this), _, 7, _, _, _) }
...
class ShortType extends IntegralType {
  ShortType() {
    cpp_builtintypes(underlyingElement(this), _, 8, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 9, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 10, _, _, _)
  }
...
class IntType extends IntegralType {
  IntType() {
    cpp_builtintypes(underlyingElement(this), _, 11, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 12, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 13, _, _, _)
  }
...
class LongType extends IntegralType {
  LongType() {
    cpp_builtintypes(underlyingElement(this), _, 14, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 15, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 16, _, _, _)
  }
...
class LongLongType extends IntegralType {
  LongLongType() {
    cpp_builtintypes(underlyingElement(this), _, 17, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 18, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 19, _, _, _)
  }
...
class Int128Type extends IntegralType {
  Int128Type() {
    cpp_builtintypes(underlyingElement(this), _, 35, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 36, _, _, _) or
    cpp_builtintypes(underlyingElement(this), _, 37, _, _, _)
  }
...
class FloatingPointType extends ArithmeticType {
  ...
  FloatingPointType() {
    exists(int kind |
      cpp_builtintypes(underlyingElement(this), _, kind, _, _, _) and
  ...
  final RealNumberType getRealType() {
    cpp_builtintypes(unresolveElement(result), _, realKind, _, _, _)
  }
...
class FloatType extends RealNumberType, BinaryFloatingPointType {
  FloatType() { cpp_builtintypes(underlyingElement(this), _, 24, _, _, _) }
...
class DoubleType extends RealNumberType, BinaryFloatingPointType {
  DoubleType() { cpp_builtintypes(underlyingElement(this), _, 25, _, _, _) }
...
class LongDoubleType extends RealNumberType, BinaryFloatingPointType {
  LongDoubleType() { cpp_builtintypes(underlyingElement(this), _, 26, _, _, _) }
...
class Float128Type extends RealNumberType, BinaryFloatingPointType {
  Float128Type() { cpp_builtintypes(underlyingElement(this), _, 38, _, _, _) }
...
class Decimal32Type extends RealNumberType, DecimalFloatingPointType {
  Decimal32Type() { cpp_builtintypes(underlyingElement(this), _, 40, _, _, _) }
...
class Decimal64Type extends RealNumberType, DecimalFloatingPointType {
  Decimal64Type() { cpp_builtintypes(underlyingElement(this), _, 41, _, _, _) }
...
class Decimal128Type extends RealNumberType, DecimalFloatingPointType {
  Decimal128Type() { cpp_builtintypes(underlyingElement(this), _, 42, _, _, _) }
...
class VoidType extends BuiltInType {
  VoidType() { cpp_builtintypes(underlyingElement(this), _, 3, _, _, _) }
...
class WideCharType extends IntegralType {
  WideCharType() { cpp_builtintypes(underlyingElement(this), _, 33, _, _, _) }
...
class Char8Type extends IntegralType {
  Char8Type() { cpp_builtintypes(underlyingElement(this), _, 51, _, _, _) }
...
class Char16Type extends IntegralType {
  Char16Type() { cpp_builtintypes(underlyingElement(this), _, 43, _, _, _) }
...
class Char32Type extends IntegralType {
  Char32Type() { cpp_builtintypes(underlyingElement(this), _, 44, _, _, _) }
...
class NullPointerType extends BuiltInType {
  NullPointerType() { cpp_builtintypes(underlyingElement(this), _, 34, _, _, _) }
 ...
 * ```
 */
class DerivedType extends Type, @cpp_derivedtype {
  override string toString() { result = this.getName() }

  override string getName() { cpp_derivedtypes(underlyingElement(this), result, _, _) }
   ...
   */
  Type getBaseType() { cpp_derivedtypes(underlyingElement(this), _, _, unresolveElement(result)) }
 ...
 * ```
 */
class Decltype extends Type, @cpp_decltype {
  override string getAPrimaryQlClass() { result = "Decltype" }

  /**
   * The expression whose type is being obtained by this decltype.
   */
  Expr getExpr() { cpp_decltypes(underlyingElement(this), unresolveElement(result), _, _) }

  /**
   * The type immediately yielded by this decltype.
   */
  Type getBaseType() { cpp_decltypes(underlyingElement(this), _, unresolveElement(result), _) }
   ...
   */
  predicate parenthesesWouldChangeMeaning() { cpp_decltypes(underlyingElement(this), _, _, true) }
...
class PointerType extends DerivedType {
  PointerType() { cpp_derivedtypes(underlyingElement(this), _, 1, _) }
...
class ReferenceType extends DerivedType {
  ReferenceType() {
    cpp_derivedtypes(underlyingElement(this), _, 2, _) or cpp_derivedtypes(underlyingElement(this), _, 8, _)
  }
...
class LValueReferenceType extends ReferenceType {
  LValueReferenceType() { cpp_derivedtypes(underlyingElement(this), _, 2, _) }
...
class RValueReferenceType extends ReferenceType {
  RValueReferenceType() { cpp_derivedtypes(underlyingElement(this), _, 8, _) }
...
class SpecifiedType extends DerivedType {
  SpecifiedType() { cpp_derivedtypes(underlyingElement(this), _, 3, _) }
...
class ArrayType extends DerivedType {
  ArrayType() { cpp_derivedtypes(underlyingElement(this), _, 4, _) }
   ...
   */
  predicate hasArraySize() { cpp_arraysizes(underlyingElement(this), _, _, _) }
   ...
   */
  int getArraySize() { cpp_arraysizes(underlyingElement(this), result, _, _) }
   ...
   */
  int getByteSize() { cpp_arraysizes(underlyingElement(this), _, result, _) }

  override int getAlignment() { cpp_arraysizes(underlyingElement(this), _, _, result) }
...
class GNUVectorType extends DerivedType {
  GNUVectorType() { cpp_derivedtypes(underlyingElement(this), _, 5, _) }
   ...
   * of a single element.
   */
  int getNumElements() { cpp_arraysizes(underlyingElement(this), result, _, _) }
   ...
   */
  override int getSize() { cpp_arraysizes(underlyingElement(this), _, result, _) }
...
class FunctionPointerType extends FunctionPointerIshType {
  FunctionPointerType() { cpp_derivedtypes(underlyingElement(this), _, 6, _) }
...
class FunctionReferenceType extends FunctionPointerIshType {
  FunctionReferenceType() { cpp_derivedtypes(underlyingElement(this), _, 7, _) }
...
class BlockType extends FunctionPointerIshType {
  BlockType() { cpp_derivedtypes(underlyingElement(this), _, 10, _) }
...
class FunctionPointerIshType extends DerivedType {
  FunctionPointerIshType() {
    cpp_derivedtypes(underlyingElement(this), _, 6, _) or
    cpp_derivedtypes(underlyingElement(this), _, 7, _) or
    cpp_derivedtypes(underlyingElement(this), _, 10, _)
  }

  /** the return type of this function pointer type */
  Type getReturnType() {
    exists(RoutineType t |
      cpp_derivedtypes(underlyingElement(this), _, _, unresolveElement(t)) and
 ...
 * ```
 */
class PointerToMemberType extends Type, @cpp_ptrtomember {
  ...
  Type getBaseType() { cpp_ptrtomembers(underlyingElement(this), unresolveElement(result), _) }

  /** the class referred by this pointer to member type */
  Type getClass() { cpp_ptrtomembers(underlyingElement(this), _, unresolveElement(result)) }
 ...
 * ```
 */
class RoutineType extends Type, @cpp_routinetype {
  /** a printable representation of this named element */
  override string toString() { result = this.getName() }
  ...
  Type getParameterType(int n) {
    cpp_routinetypeargs(underlyingElement(this), n, unresolveElement(result))
  }

  /**
   * Gets the type of a parameter to this routine.
   */
  Type getAParameterType() { cpp_routinetypeargs(underlyingElement(this), _, unresolveElement(result)) }

  /**
   * Gets the return type of this routine.
   */
  Type getReturnType() { cpp_routinetypes(underlyingElement(this), unresolveElement(result)) }
...
class TemplateParameter extends UserType {
  TemplateParameter() {
    cpp_usertypes(underlyingElement(this), _, 7) or cpp_usertypes(underlyingElement(this), _, 8)
  }
...
class TemplateTemplateParameter extends TemplateParameter {
  TemplateTemplateParameter() { cpp_usertypes(underlyingElement(this), _, 8) }
...
class AutoType extends TemplateParameter {
  AutoType() { cpp_usertypes(underlyingElement(this), "auto", 7) }
...
class TypeMention extends Locatable, @cpp_type_mention {
   ...
   */
  Type getMentionedType() { cpp_type_mentions(underlyingElement(this), unresolveElement(result), _, _) }

  override Location getLocation() { cpp_type_mentions(underlyingElement(this), _, result, _) }
}
