 ...
 */
class LambdaExpression extends Expr, @cpp_lambdaexpr {
  ...
  LambdaCapture getCapture(int index) {
    cpp_lambda_capture(result, underlyingElement(this), index, _, _, _, _)
  }
   ...
   */
  string getDefaultCaptureMode() { cpp_lambdas(underlyingElement(this), result, _) }

  /**
   * Holds if the return type (of the call operator of the resulting object) was explicitly specified.
   */
  predicate returnTypeIsExplicit() { cpp_lambdas(underlyingElement(this), _, true) }
 ...
 * ```
 */
class LambdaCapture extends Locatable, @cpp_lambdacapture {
   ...
   */
  predicate isImplicit() { cpp_lambda_capture(this, _, _, _, _, true, _) }
   ...
   */
  predicate isCapturedByReference() { cpp_lambda_capture(this, _, _, _, true, _, _) }
   ...
   */
  override Location getLocation() { cpp_lambda_capture(this, _, _, _, _, _, result) }

  /**
   * Gets the field of the lambda expression's closure type which is used to store this capture.
   */
  MemberVariable getField() { cpp_lambda_capture(this, _, _, result, _, _, _) }
