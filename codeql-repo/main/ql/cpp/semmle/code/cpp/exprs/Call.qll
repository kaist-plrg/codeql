...
private class TCall = @cpp_funbindexpr or @cpp_callexpr;

/**
 * A C/C++ call.
 */
class Call extends Expr, NameQualifiableElement, TCall {
  // `@cpp_funbindexpr` (which is the dbscheme type for FunctionCall) is a union type that includes
  // `@cpp_routineexpr. This dbscheme type includes accesses to functions that are not necessarily calls to
  // that function. That's why the charpred for `FunctionCall` requires:
  // ```
  // cpp_iscall(underlyingElement(this), _)
  // ```
  // So for the charpred for `Call` we include the requirement that if this is an instance of
  // `@cpp_funbindexpr` it must be a _call_ to the function.
  Call() { this instanceof @cpp_callexpr or cpp_iscall(underlyingElement(this), _) }
 ...
 */
class FunctionCall extends Call, @cpp_funbindexpr {
  FunctionCall() { cpp_iscall(underlyingElement(this), _) }
  ...
  int getNumberOfExplicitTemplateArguments() {
    if cpp_numtemplatearguments(underlyingElement(this), _)
    then cpp_numtemplatearguments(underlyingElement(this), result)
    else result = 0
  }
  ...
  predicate hasTemplateArgumentList() { cpp_numtemplatearguments(underlyingElement(this), _) }
   ...
   */
  override Function getTarget() { cpp_funbind(underlyingElement(this), unresolveElement(result)) }
   ...
   */
  predicate isVirtual() { cpp_iscall(underlyingElement(this), 1) }
   ...
   */
  predicate isOnlyFoundByADL() { cpp_iscall(underlyingElement(this), 2) }
 ...
 * ```
 */
class ExprCall extends Call, @cpp_callexpr {
  /**
 ...
 * ```
 */
class VacuousDestructorCall extends Expr, @cpp_vacuous_destructor_call {
  /**
 ...
 * initializations.
 */
class ConstructorInit extends Expr, @cpp_ctorinit {
 ...
 * ```
 */
class ConstructorDirectInit extends ConstructorBaseInit, @cpp_ctordirectinit {
 ...
 * ```
 */
class ConstructorVirtualInit extends ConstructorBaseInit, @cpp_ctorvirtualinit {
 ...
 * ```
 */
class ConstructorDelegationInit extends ConstructorBaseInit, @cpp_ctordelegatinginit {
 ...
 * ```
 */
class ConstructorFieldInit extends ConstructorInit, @cpp_ctorfieldinit {
  /** Gets the field being initialized. */
  Field getTarget() { cpp_varbind(underlyingElement(this), unresolveElement(result)) }
 ...
 */
class DestructorDestruction extends Expr, @cpp_dtordestruct {
 ...
 * ```
 */
class DestructorDirectDestruction extends DestructorBaseDestruction, @cpp_dtordirectdestruct {
 ...
 * ```
 */
class DestructorVirtualDestruction extends DestructorBaseDestruction, @cpp_dtorvirtualdestruct {
 ...
 * ```
 */
class DestructorFieldDestruction extends DestructorDestruction, @cpp_dtorfielddestruct {
