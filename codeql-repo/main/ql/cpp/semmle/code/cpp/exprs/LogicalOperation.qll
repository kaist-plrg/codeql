 ...
 */
class UnaryLogicalOperation extends UnaryOperation, @cpp_un_log_op_expr { }
 ...
 * ```
 */
class NotExpr extends UnaryLogicalOperation, @cpp_notexpr {
 ...
 */
class BinaryLogicalOperation extends BinaryOperation, @cpp_bin_log_op_expr {
  /**
 ...
 * ```
 */
class LogicalAndExpr extends BinaryLogicalOperation, @cpp_andlogicalexpr {
  override string getOperator() { result = "&&" }
 ...
 * ```
 */
class LogicalOrExpr extends BinaryLogicalOperation, @cpp_orlogicalexpr {
  override string getOperator() { result = "||" }
 ...
 * ```
 */
class ConditionalExpr extends Operation, @cpp_conditionalexpr {
  /** Gets the condition of this conditional expression. */
  Expr getCondition() { cpp_expr_cond_guard(underlyingElement(this), unresolveElement(result)) }
  ...
  Expr getThen() {
    ...
    else cpp_expr_cond_true(underlyingElement(this), unresolveElement(result))
  }

  /** Gets the 'else' expression of this conditional expression. */
  Expr getElse() { cpp_expr_cond_false(underlyingElement(this), unresolveElement(result)) }

  /**
   * Holds if this expression used the two operand form `guard ? : false`.
   */
  predicate isTwoOperand() { cpp_expr_cond_two_operand(underlyingElement(this)) }
