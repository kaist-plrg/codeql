 ...
 */
class Access extends Expr, NameQualifiableElement, @cpp_access {
  // As `@cpp_access` is a union type containing `@cpp_routineexpr` (which describes function accesses
  // that are called), we need to exclude function calls.
  Access() { this instanceof @cpp_routineexpr implies not cpp_iscall(underlyingElement(this), _) }
 ...
 * ```
 */
class EnumConstantAccess extends Access, @cpp_varaccess {
  override string getAPrimaryQlClass() { result = "EnumConstantAccess" }

  EnumConstantAccess() {
    exists(EnumConstant c | cpp_varbind(underlyingElement(this), unresolveElement(c)))
  }

  /** Gets the accessed `enum` constant. */
  override EnumConstant getTarget() { cpp_varbind(underlyingElement(this), unresolveElement(result)) }
 ...
 * ```
 */
class VariableAccess extends Access, @cpp_varaccess {
  override string getAPrimaryQlClass() { result = "VariableAccess" }

  VariableAccess() {
    not exists(EnumConstant c | cpp_varbind(underlyingElement(this), unresolveElement(c)))
  }

  /** Gets the accessed variable. */
  override Variable getTarget() { cpp_varbind(underlyingElement(this), unresolveElement(result)) }
...
class FieldAccess extends VariableAccess {
  ...
  FieldAccess() { exists(Field f | cpp_varbind(underlyingElement(this), unresolveElement(f))) }
 ...
 * ```
 */
class FunctionAccess extends Access, @cpp_routineexpr {
  FunctionAccess() { not cpp_iscall(underlyingElement(this), _) }
 ...
 */
class ParamAccessForType extends Expr, @cpp_param_ref {
 ...
 * ```
 */
class TypeName extends Expr, @cpp_type_operand {
 ...
 * `OverloadedArrayExpr`.
 */
class ArrayExpr extends Expr, @cpp_subscriptexpr {
