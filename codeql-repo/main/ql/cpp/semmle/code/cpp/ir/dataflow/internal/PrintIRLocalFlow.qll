...
private string getNodeProperty(DataFlow::Node node, string key) {
  // List dataflow into and out of this node. Flow into this node is printed as `src->@cpp_`, and flow
  // out of this node is printed as `@cpp_->dest`.
  key = "flow" and
  result =
    strictconcat(string flow, boolean to, int order1, int order2 |
      flow = getFromFlow(node, order1, order2) + "->@cpp_" and to = false
      or
      flow = "@cpp_->" + getToFlow(node, order1, order2) and to = true
    |
