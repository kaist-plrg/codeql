 ...
 */
class Function extends Declaration, ControlFlowNode, AccessHolder, @cpp_function {
  override string getName() { cpp_functions(underlyingElement(this), result, _) }
  ...
  override Specifier getASpecifier() {
    cpp_funspecifiers(underlyingElement(this), unresolveElement(result)) or
    result.hasName(getADeclarationEntry().getASpecifier())
  }

  /** Gets an attribute of this function. */
  Attribute getAnAttribute() { cpp_funcattributes(underlyingElement(this), unresolveElement(result)) }
   ...
   */
  predicate isDeleted() { cpp_function_deleted(underlyingElement(this)) }

  /**
   * Holds if this function is explicitly defaulted with the `= default`
   * specifier.
   */
  predicate isDefaulted() { cpp_function_defaulted(underlyingElement(this)) }
  ...
  Type getType() { cpp_function_return_type(underlyingElement(this), unresolveElement(result)) }
   ...
   */
  Parameter getParameter(int n) { cpp_params(unresolveElement(result), underlyingElement(this), n, _) }
   ...
   */
  Parameter getAParameter() { cpp_params(unresolveElement(result), underlyingElement(this), _, _) }
  ...
  override FunctionDeclarationEntry getADeclarationEntry() {
    if cpp_fun_decls(_, underlyingElement(this), _, _, _)
    then declEntry(result)
    else
      exists(Function f |
        this.isConstructedFrom(f) and
        cpp_fun_decls(unresolveElement(result), unresolveElement(f), _, _, _)
      )
  }

  private predicate declEntry(FunctionDeclarationEntry fde) {
    cpp_fun_decls(unresolveElement(fde), underlyingElement(this), _, _, _) and
   ...
   * `FunctionTryStmt`.
   */
  Stmt getEntryPoint() { cpp_function_entry_point(underlyingElement(this), unresolveElement(result)) }
  ...
  predicate isConstructedFrom(Function f) {
    cpp_function_instantiation(underlyingElement(this), unresolveElement(f))
  }
 ...
 * ```
 */
class FunctionDeclarationEntry extends DeclarationEntry, @cpp_fun_decl {
  ...
  /** Gets the function which is being declared or defined. */
  Function getFunction() { cpp_fun_decls(underlyingElement(this), unresolveElement(result), _, _, _) }

  /** Gets the name of the function. */
  override string getName() { cpp_fun_decls(underlyingElement(this), _, _, result, _) }

  /**
   * Gets the return type of the function which is being declared or
   * defined.
   */
  override Type getType() { cpp_fun_decls(underlyingElement(this), _, unresolveElement(result), _, _) }

  /** Gets the location of this declaration entry. */
  override Location getLocation() { cpp_fun_decls(underlyingElement(this), _, _, _, result) }

  /** Gets a specifier associated with this declaration entry. */
  override string getASpecifier() { cpp_fun_decl_specifiers(underlyingElement(this), result) }
  ...
  TypedefType getTypedefType() {
    cpp_fun_decl_typedef_type(underlyingElement(this), unresolveElement(result))
  }
  ...
  ParameterDeclarationEntry getParameterDeclarationEntry(int n) {
    cpp_param_decl_bind(unresolveElement(result), n, underlyingElement(this))
  }
  ...
  override predicate isDefinition() { cpp_fun_def(underlyingElement(this)) }

  /** Holds if this declaration is a Template specialization. */
  predicate isSpecialization() { cpp_fun_specialized(underlyingElement(this)) }
   ...
   */
  predicate isImplicit() { cpp_fun_implicit(underlyingElement(this)) }
  ...
  Type getThrownType(int i) {
    cpp_fun_decl_throws(underlyingElement(this), i, unresolveElement(result))
  }
   ...
   */
  Expr getNoExceptExpr() { cpp_fun_decl_noexcept(underlyingElement(this), unresolveElement(result)) }
  ...
  predicate hasExceptionSpecification() {
    cpp_fun_decl_throws(underlyingElement(this), _, _) or
    cpp_fun_decl_noexcept(underlyingElement(this), _) or
    isNoThrow() or
    isNoExcept()
  }
   ...
   */
  predicate isNoThrow() { cpp_fun_decl_empty_throws(underlyingElement(this)) }

  /**
   * Holds if the declared function has an empty `noexcept` exception
   * specification.
   */
  predicate isNoExcept() { cpp_fun_decl_empty_noexcept(underlyingElement(this)) }
}
...
class Operator extends Function {
  Operator() { cpp_functions(underlyingElement(this), _, 5) }
...
class TemplateFunction extends Function {
  TemplateFunction() {
    cpp_is_function_template(underlyingElement(this)) and exists(getATemplateArgument())
  }
...
class BuiltInFunction extends Function {
  BuiltInFunction() { cpp_functions(underlyingElement(this), _, 6) }
