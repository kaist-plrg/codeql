...
class Variable extends Declaration, @cpp_variable {
  ...
  override Specifier getASpecifier() {
    cpp_varspecifiers(underlyingElement(this), unresolveElement(result))
  }

  /** Gets an attribute of this variable. */
  Attribute getAnAttribute() { cpp_varattributes(underlyingElement(this), unresolveElement(result)) }
   ...
   */
  Type getTypeWithAuto() { cpp_autoderivation(underlyingElement(this), unresolveElement(result)) }

  /**
   * Holds if the type of this variable is declared using the C++ `auto`
   * keyword.
   */
  predicate declaredUsingAutoType() { cpp_autoderivation(underlyingElement(this), _) }
  ...
  predicate isConstructedFrom(Variable v) {
    cpp_variable_instantiation(underlyingElement(this), unresolveElement(v))
  }
   ...
   *    `for (char c : str) { ... }`
   */
  predicate isCompilerGenerated() { cpp_compgenerated(underlyingElement(this)) }
}
 ...
 * ```
 */
class VariableDeclarationEntry extends DeclarationEntry, @cpp_var_decl {
   ...
   */
  Variable getVariable() { cpp_var_decls(underlyingElement(this), unresolveElement(result), _, _, _) }
   ...
   * ```
   */
  override string getName() { cpp_var_decls(underlyingElement(this), _, _, result, _) and result != "" }

  /**
   * Gets the type of the variable which is being declared or defined.
   */
  override Type getType() { cpp_var_decls(underlyingElement(this), _, unresolveElement(result), _, _) }

  override Location getLocation() { cpp_var_decls(underlyingElement(this), _, _, _, result) }
   ...
   * a definition.
   */
  override predicate isDefinition() { cpp_var_def(underlyingElement(this)) }

  override string getASpecifier() { cpp_var_decl_specifiers(underlyingElement(this), result) }
}
...
class ParameterDeclarationEntry extends VariableDeclarationEntry {
  ParameterDeclarationEntry() { cpp_param_decl_bind(underlyingElement(this), _, _) }
  ...
  FunctionDeclarationEntry getFunctionDeclarationEntry() {
    cpp_param_decl_bind(underlyingElement(this), _, unresolveElement(result))
  }

  /**
   * Gets the zero-based index of this parameter.
   */
  int getIndex() { cpp_param_decl_bind(underlyingElement(this), result, _) }
...
class LocalVariable extends LocalScopeVariable, @cpp_localvariable {
  ...
  override string getName() { cpp_localvariables(underlyingElement(this), _, result) }

  override Type getType() { cpp_localvariables(underlyingElement(this), unresolveElement(result), _) }
...
class GlobalOrNamespaceVariable extends Variable, @cpp_globalvariable {
  override string getName() { cpp_globalvariables(underlyingElement(this), _, result) }

  override Type getType() { cpp_globalvariables(underlyingElement(this), unresolveElement(result), _) }
...
class NamespaceVariable extends GlobalOrNamespaceVariable {
  NamespaceVariable() {
    exists(Namespace n | cpp_namespacembrs(unresolveElement(n), underlyingElement(this)))
  }
 ...
 */
class MemberVariable extends Variable, @cpp_membervariable {
  ...
  override string getName() { cpp_membervariables(underlyingElement(this), _, result) }
  ...
  private Type getAType() { cpp_membervariables(underlyingElement(this), unresolveElement(result), _) }
}
...
class TemplateVariable extends Variable {
  TemplateVariable() { cpp_is_variable_template(underlyingElement(this)) }
