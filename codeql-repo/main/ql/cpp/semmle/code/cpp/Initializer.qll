 ...
 * ```
 */
class Initializer extends ControlFlowNode, @cpp_initialiser {
  override Location getLocation() { cpp_initialisers(underlyingElement(this), _, _, result) }
  ...
  Declaration getDeclaration() {
    cpp_initialisers(underlyingElement(this), unresolveElement(result), _, _)
  }

  /** Gets the initializing expression. */
  Expr getExpr() { cpp_initialisers(underlyingElement(this), _, unresolveElement(result), _) }
