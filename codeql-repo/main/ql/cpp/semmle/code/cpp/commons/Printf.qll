...
class FormatLiteral extends Literal {
  ...
  /**
   * Gets the format string, with '%%' and '%@cpp_' replaced by '_' (to avoid processing
   * them as format specifiers).
   */
  string getFormat() { result = this.getValue().replaceAll("%%", "_").replaceAll("%@cpp_", "_") }
  ...
  private string getConvCharRegexp() {
    if isMicrosoft()
    then result = "[aAcCdeEfFgGimnopsSuxXZ@cpp_]"
    else result = "[aAcCdeEfFgGimnopsSuxX@cpp_]"
  }
