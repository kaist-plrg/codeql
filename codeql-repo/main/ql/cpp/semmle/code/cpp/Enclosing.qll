...
Element exprEnclosingElement(Expr e) {
  ...
  or
  exists(Expr anc |
    cpp_expr_ancestor(unresolveElement(e), unresolveElement(anc)) and result = exprEnclosingElement(anc)
  )
  or
  exists(Stmt anc |
    cpp_expr_ancestor(unresolveElement(e), unresolveElement(anc)) and result = stmtEnclosingElement(anc)
  )
  or
  exists(DeclarationEntry de |
    cpp_expr_ancestor(unresolveElement(e), unresolveElement(de)) and
