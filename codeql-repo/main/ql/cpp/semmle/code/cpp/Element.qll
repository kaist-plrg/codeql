...
/**
 * Get the `Element` that represents this `@cpp_element`.
 ...
 */
pragma[inline]
Element mkElement(@cpp_element e) { unresolveElement(result) = e }

/**
 * INTERNAL: Do not use.
 *
 * Gets an `@cpp_element` that resolves to the `Element`. This should
 ...
 */
pragma[inline]
@cpp_element unresolveElement(Element e) {
  not result instanceof @cpp_usertype and
  result = e
  or
 ...
 * INTERNAL: Do not use.
 *
 * Gets the `@cpp_element` that this `Element` extends. This should normally
 ...
 */
@cpp_element underlyingElement(Element e) { result = e }
 ...
 */
class ElementBase extends @cpp_element {
...
class Element extends ElementBase {
  ...
  Element getParentScope() {
    ...
    exists(BlockStmt b | this = b and cpp_blockscope(unresolveElement(b), unresolveElement(result)))
    or
    ...
    or
    cpp_using_container(unresolveElement(result), underlyingElement(this))
  }
  ...
  private Element getEnclosingElementPref() {
    cpp_enclosingfunction(underlyingElement(this), unresolveElement(result)) or
    result.(Function) = stmtEnclosingElement(this) or
    this.(LocalScopeVariable).getFunction() = result or
    cpp_enumconstants(underlyingElement(this), unresolveElement(result), _, _, _, _) or
    cpp_derivations(underlyingElement(this), unresolveElement(result), _, _, _) or
    cpp_stmtparents(underlyingElement(this), _, unresolveElement(result)) or
    cpp_exprparents(underlyingElement(this), _, unresolveElement(result)) or
    cpp_namequalifiers(underlyingElement(this), unresolveElement(result), _, _) or
    cpp_initialisers(underlyingElement(this), unresolveElement(result), _, _) or
    cpp_exprconv(unresolveElement(result), underlyingElement(this)) or
    cpp_param_decl_bind(underlyingElement(this), _, unresolveElement(result)) or
    cpp_using_container(unresolveElement(result), underlyingElement(this)) or
    cpp_static_asserts(unresolveElement(this), _, _, _, underlyingElement(result))
  }
  ...
  Element getEnclosingElement() {
    ...
    (
      ...
      or
      cpp_var_decls(underlyingElement(this), unresolveElement(result), _, _, _)
    )
  }
...
private predicate isFromUninstantiatedTemplateRec(Element e, Element template) {
  cpp_is_class_template(unresolveElement(template)) and
  e = template
  or
  cpp_is_function_template(unresolveElement(template)) and
  e = template
  or
  cpp_is_variable_template(unresolveElement(template)) and
  e = template
  or
 ...
 */
class StaticAssert extends Locatable, @cpp_static_assert {
  override string toString() { result = "static_assert(..., \"" + getMessage() + "\")" }

  /**
   * Gets the expression which this static assertion ensures is true.
   */
  Expr getCondition() { cpp_static_asserts(underlyingElement(this), unresolveElement(result), _, _, _) }

  /**
   * Gets the message which will be reported by the compiler if this static assertion fails.
   */
  string getMessage() { cpp_static_asserts(underlyingElement(this), _, result, _, _) }

  override Location getLocation() { cpp_static_asserts(underlyingElement(this), _, _, result, _) }
}
