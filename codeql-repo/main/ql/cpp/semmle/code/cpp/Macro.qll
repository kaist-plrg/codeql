 ...
 * ```
 */
class Macro extends PreprocessorDirective, @cpp_ppd_define {
  /**
   * Gets the head of this macro. For example, `MAX(x,y)` in
   * `#define MAX(x,y) (((x)>(y))?(x):(y))`.
   */
  override string getHead() { cpp_preproctext(underlyingElement(this), result, _) }
   ...
   * `#define MAX(x,y) (((x)>(y))?(x):(y))`.
   */
  string getBody() { cpp_preproctext(underlyingElement(this), _, result) }
 ...
 */
class MacroAccess extends Locatable, @cpp_macroinvocation {
  /** Gets the macro that is being accessed. */
  Macro getMacro() { cpp_macroinvocations(underlyingElement(this), unresolveElement(result), _, _) }
   ...
   */
  Location getActualLocation() { cpp_macroinvocations(underlyingElement(this), _, result, _) }
  ...
  MacroInvocation getParentInvocation() {
    cpp_macroparent(underlyingElement(this), unresolveElement(result))
  }
...
class MacroInvocation extends MacroAccess {
  MacroInvocation() { cpp_macroinvocations(underlyingElement(this), _, _, 1) }
  ...
  Locatable getAnExpandedElement() {
    cpp_inmacroexpansion(unresolveElement(result), underlyingElement(this))
  }
  ...
  Locatable getAnAffectedElement() {
    cpp_inmacroexpansion(unresolveElement(result), underlyingElement(this)) or
    cpp_macrolocationbind(underlyingElement(this), result.getLocation())
  }
  ...
  string getUnexpandedArgument(int i) {
    cpp_macro_argument_unexpanded(underlyingElement(this), i, result)
  }
   ...
   */
  string getExpandedArgument(int i) { cpp_macro_argument_expanded(underlyingElement(this), i, result) }
}
...
predicate macroLocation(Location l) { cpp_macrolocationbind(_, l) }

/** Holds if `element` is in the expansion of a macro. */
predicate inMacroExpansion(Locatable element) {
  cpp_inmacroexpansion(unresolveElement(element), _)
  or
...
predicate affectedByMacro(Locatable element) {
  ...
  cpp_affectedbymacroexpansion(unresolveElement(element), _)
}
