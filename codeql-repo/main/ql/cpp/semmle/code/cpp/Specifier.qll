 ...
 */
class Specifier extends Element, @cpp_specifier {
  ...
  string getName() { cpp_specifiers(underlyingElement(this), result) }
 ...
 */
class Attribute extends Element, @cpp_attribute {
  /**
   ...
   */
  string getName() { cpp_attributes(underlyingElement(this), _, result, _, _) }

  override Location getLocation() { cpp_attributes(underlyingElement(this), _, _, _, result) }
 ...
 */
class GnuAttribute extends Attribute, @cpp_gnuattribute { }

/**
 * An attribute introduced by the C++11 standard `[[name]]` syntax, for
 * example: `[[clang::fallthrough]]`.
 */
class StdAttribute extends Attribute, @cpp_stdattribute {
  /**
   ...
   * `[[clang::fallthrough]]`.
   */
  string getNamespace() { cpp_attributes(underlyingElement(this), _, _, result, _) }
 ...
 */
class Declspec extends Attribute, @cpp_declspec { }

/**
 * An attribute introduced by Microsoft's "[name]" syntax, for example "[SA_Pre(Deref=1,Access=SA_Read)]".
 */
class MicrosoftAttribute extends Attribute, @cpp_msattribute {
 ...
 */
class AlignAs extends Attribute, @cpp_alignas {
 ...
 */
class AttributeArgument extends Element, @cpp_attribute_arg {
  /**
   ...
   */
  string getName() { cpp_attribute_arg_name(underlyingElement(this), result) }

  /**
   * Gets the text for the value of this argument, if its value is
   * a string or a number.
   */
  string getValueText() { cpp_attribute_arg_value(underlyingElement(this), result) }
   ...
   */
  Type getValueType() { cpp_attribute_arg_type(underlyingElement(this), unresolveElement(result)) }

  /**
   * Gets the attribute to which this is an argument.
   */
  Attribute getAttribute() {
    cpp_attribute_args(underlyingElement(this), _, unresolveElement(result), _, _)
  }
   ...
   */
  int getIndex() { cpp_attribute_args(underlyingElement(this), _, _, result, _) }

  override Location getLocation() { cpp_attribute_args(underlyingElement(this), _, _, _, result) }

  override string toString() {
    if exists(@cpp_attribute_arg_empty self | self = underlyingElement(this))
    then result = "empty argument"
    else
      exists(string prefix, string tail |
        (if exists(getName()) then prefix = getName() + "=" else prefix = "") and
        (
          if exists(@cpp_attribute_arg_type self | self = underlyingElement(this))
