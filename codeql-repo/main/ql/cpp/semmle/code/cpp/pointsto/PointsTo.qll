...
predicate lvalue(Element e) {
  ...
  exists(ParenthesisExpr paren |
    cpp_exprconv(unresolveElement(e), unresolveElement(paren)) and lvalue(paren)
  )
  or
...
private predicate pointerEntity(@cpp_element src, @cpp_element dest) {
...
predicate pointer(Element src, Element dest) {
  ...
  exists(ReferenceDereferenceExpr deref |
    not lvalue(deref) and
    dest = deref and
    cpp_exprconv(unresolveElement(src), unresolveElement(deref))
  )
  or
...
private predicate flowEntity(@cpp_element src, @cpp_element dest) { flow(mkElement(src), mkElement(dest)) }
...
predicate flow(Element src, Element dest) {
  ...
  exists(ReferenceToExpr toref |
    cpp_exprconv(unresolveElement(src), unresolveElement(toref)) and dest = toref
  )
  or
  exists(ReferenceDereferenceExpr deref |
    lvalue(deref) and
  ...
  exists(ArrayToPointerConversion conv |
    cpp_exprconv(unresolveElement(src), unresolveElement(conv)) and dest = conv
  )
  or
  exists(ParenthesisExpr paren |
    // these can appear on the LHS of an assignment
    cpp_exprconv(unresolveElement(src), unresolveElement(paren)) and dest = paren
    or
    cpp_exprconv(unresolveElement(dest), unresolveElement(paren)) and src = paren
  )
  or
  ...
  or
  exists(ParenthesisExpr paren |
    dest = paren and cpp_exprconv(unresolveElement(src), unresolveElement(paren))
  )
  or
...
private predicate compoundEdgeEntity(
  @cpp_element parent, @cpp_element element, string label, @cpp_element other, int kind
) {
 ...
 */
cached
predicate pointstoinfo(int parent, @cpp_element elem, string label, int ptset) =
 ...
 */
cached
predicate pointstosets(int ptset, @cpp_element elem) { pointstoinfo(_, elem, "", ptset) }
 ...
 */
pragma[noopt]
predicate parentSetFor(int cset, @cpp_element expr) {
  exists(string s | s = "" and pointstoinfo(cset, expr, s, _))
}

private predicate locationEntity(@cpp_element location) { location(mkElement(location)) }
 ...
 */
cached
predicate setlocations(int set, @cpp_element location) {
...
class PointsToExpr extends Expr {
  ...
  Element pointsTo() {
    ...
    exists(int set, @cpp_element thisEntity, @cpp_element resultEntity |
...
private @cpp_element localUnresolveElement(Element e) { result = unresolveElement(e) }
