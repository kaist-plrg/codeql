...
/** A file or folder. */
class Container extends Locatable, @cpp_container {
  /**
  ...
  Container getParentContainer() {
    cpp_containerparent(unresolveElement(result), underlyingElement(this))
  }
 ...
 */
class Folder extends Container, @cpp_folder {
  override string getAbsolutePath() { cpp_folders(underlyingElement(this), result, _) }
   ...
   */
  deprecated string getName() { cpp_folders(underlyingElement(this), result, _) }
  ...
  deprecated string getShortName() {
    exists(string longnameRaw, string longname |
      cpp_folders(underlyingElement(this), _, longnameRaw) and
 ...
 */
class File extends Container, @cpp_file {
  override string getAbsolutePath() { cpp_files(underlyingElement(this), result, _, _, _) }
  ...
  predicate compiledAsC() { cpp_fileannotations(underlyingElement(this), 1, "compiled as c", "1") }

  /** Holds if this file was compiled as C++ (at any point). */
  predicate compiledAsCpp() { cpp_fileannotations(underlyingElement(this), 1, "compiled as c++", "1") }
   ...
   */
  override predicate fromSource() { cpp_numlines(underlyingElement(this), _, _, _) }
   ...
   */
  string getExtensions() { cpp_files(underlyingElement(this), _, _, result, _) }
   ...
   */
  string getShortName() { cpp_files(underlyingElement(this), _, result, _, _) }
}
