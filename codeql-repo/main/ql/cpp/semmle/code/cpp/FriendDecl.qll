...
class FriendDecl extends Declaration, @cpp_frienddecl {
  ...
  override Location getLocation() { cpp_frienddecls(underlyingElement(this), _, _, result) }
   ...
   */
  AccessHolder getFriend() { cpp_frienddecls(underlyingElement(this), _, unresolveElement(result), _) }

  /**
   * Gets the declaring class (also known as the befriending class).
   * For example: `A` in `class A { friend class X }`.
   */
  Class getDeclaringClass() { cpp_frienddecls(underlyingElement(this), unresolveElement(result), _, _) }
