...
class Enum extends UserType, IntegralOrEnumType {
  ...
  EnumConstant getEnumConstant(int index) {
    cpp_enumconstants(unresolveElement(result), underlyingElement(this), index, _, _, _)
  }
   ...
   */
  predicate hasExplicitUnderlyingType() { cpp_derivations(_, underlyingElement(this), _, _, _) }
  ...
  Type getExplicitUnderlyingType() {
    cpp_derivations(_, underlyingElement(this), _, unresolveElement(result), _)
  }
}
...
class ScopedEnum extends Enum {
  ScopedEnum() { cpp_usertypes(underlyingElement(this), _, 13) }
...
class EnumConstant extends Declaration, @cpp_enumconstant {
  ...
  Enum getDeclaringEnum() {
    cpp_enumconstants(underlyingElement(this), unresolveElement(result), _, _, _, _)
  }
   ...
   */
  override string getName() { cpp_enumconstants(underlyingElement(this), _, _, _, result, _) }
  ...
  Type getType() { cpp_enumconstants(underlyingElement(this), _, _, unresolveElement(result), _, _) }
  ...
  override Location getDefinitionLocation() {
    cpp_enumconstants(underlyingElement(this), _, _, _, _, result)
  }
