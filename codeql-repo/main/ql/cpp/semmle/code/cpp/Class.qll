...
class Class extends UserType {
  ...
  Declaration getCanonicalMember(int index) {
    cpp_member(underlyingElement(this), index, unresolveElement(result))
  }
   ...
   */
  predicate isPOD() { cpp_is_pod_class(underlyingElement(this)) }

  /**
   * Holds if this class, struct or union is a standard-layout class
   * [N4140 9(7)]. Also holds for structs in C programs.
   */
  predicate isStandardLayout() { cpp_is_standard_layout_class(underlyingElement(this)) }
  ...
  int getVirtualBaseClassByteOffset(Class base) {
    cpp_virtual_base_offsets(underlyingElement(this), unresolveElement(base), result)
  }
   ...
   * extracted).
   */
  override int getAlignment() { cpp_usertypesize(underlyingElement(this), _, result) }
  ...
  predicate isConstructedFrom(Class c) {
    cpp_class_instantiation(underlyingElement(this), unresolveElement(c))
  }
  ...
  predicate isFinal() { cpp_usertype_final(underlyingElement(this)) }
   ...
   */
  string getUuid() { cpp_usertype_uuid(underlyingElement(this), result) }
 ...
 * ```
 */
class ClassDerivation extends Locatable, @cpp_derivation {
  /**
   ...
   * ```
   */
  Type getBaseType() { cpp_derivations(underlyingElement(this), _, _, unresolveElement(result), _) }
  ...
  Class getDerivedClass() {
    cpp_derivations(underlyingElement(this), unresolveElement(result), _, _, _)
  }
   ...
   * ```
   */
  int getIndex() { cpp_derivations(underlyingElement(this), _, result, _, _) }

  /** Gets a specifier (for example `public`) applied to the derivation. */
  Specifier getASpecifier() { cpp_derspecifiers(underlyingElement(this), unresolveElement(result)) }
  ...
  override Location getLocation() { cpp_derivations(underlyingElement(this), _, _, _, result) }
   ...
   * classes.
   */
  int getByteOffset() { cpp_direct_base_offsets(underlyingElement(this), result) }
...
class TemplateClass extends Class {
  TemplateClass() { cpp_usertypes(underlyingElement(this), _, 6) }
...
deprecated class Interface extends Class {
  Interface() {
    forex(Declaration m |
      m.getDeclaringType() = this.getABaseClass*() and not cpp_compgenerated(unresolveElement(m))
    |
...
class ProxyClass extends UserType {
  ProxyClass() { cpp_usertypes(underlyingElement(this), _, 9) }
  ...
  TemplateParameter getTemplateParameter() {
    cpp_is_proxy_class_for(underlyingElement(this), unresolveElement(result))
  }
}
