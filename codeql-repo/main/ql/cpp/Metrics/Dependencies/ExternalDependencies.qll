...
private newtype LibraryT =
  ...
  LibraryTExternalPackage(@cpp_external_package ep, string name, string version) {
    exists(string namespace, string package_name |
      cpp_external_packages(ep, namespace, package_name, version) and
      name = package_name
    )
  }
...
class Library extends LibraryT {
  ...
  Library() {
    ...
    exists(@cpp_external_package ep | this = LibraryTExternalPackage(ep, name, version))
  }
  ...
  File getAFile() {
    ...
    or
    exists(@cpp_external_package ep |
      this = LibraryTExternalPackage(ep, _, _) and
      cpp_header_to_external_package(unresolveElement(result), ep)
    )
  }
}
