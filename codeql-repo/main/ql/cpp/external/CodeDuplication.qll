...
private predicate tokenLocation(string path, int sl, int sc, int ec, int el, Copy copy, int index) {
  ...
  cpp_tokens(copy, index, sl, sc, ec, el)
}

/** A token block used for detection of duplicate and similar code. */
class Copy extends @cpp_duplication_or_similarity {
  /** Gets the index of the last token in this block. */
  private int lastToken() { result = max(int i | cpp_tokens(this, i, _, _, _, _) | i) }
  ...
  int sourceStartLine() { cpp_tokens(this, 0, result, _, _, _) }

  /** Gets the column on which the first token in this block starts. */
  int sourceStartColumn() { cpp_tokens(this, 0, _, result, _, _) }

  /** Gets the line on which the last token in this block ends. */
  int sourceEndLine() { cpp_tokens(this, lastToken(), _, _, result, _) }

  /** Gets the column on which the last token in this block ends. */
  int sourceEndColumn() { cpp_tokens(this, lastToken(), _, _, _, result) }

  /** Gets the number of lines containing at least (part of) one token in this block. */
  int sourceLines() { result = this.sourceEndLine() + 1 - this.sourceStartLine() }

  /** Gets an opaque identifier for the equivalence class of this block. */
  int getEquivalenceClass() { cpp_duplicateCode(this, _, result) or cpp_similarCode(this, _, result) }

  /** Gets the source file in which this block appears. */
  File sourceFile() {
    exists(string name | cpp_duplicateCode(this, name, _) or cpp_similarCode(this, name, _) |
...
class DuplicateBlock extends Copy, @cpp_duplication {
  override string toString() { result = "Duplicate code: " + sourceLines() + " duplicated lines." }
}

/** A block of similar code. */
class SimilarBlock extends Copy, @cpp_similarity {
...
FunctionDeclarationEntry sourceMethod() {
  ...
  cpp_numlines(unresolveElement(result.getFunction()), _, _, _)
}
  ...
  exists(int numDup, int total |
    ...
    (
      ...
      exists(string s1, string s2, string name |
        s1 = "All methods in " and s2 = " are identical in $@cpp_." and name = c.getName()
      |
        message = s1 + name + s2
      )
    )
