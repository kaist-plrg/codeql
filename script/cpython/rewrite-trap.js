...
const PYTHON_PREFIX = "python_";
const CPP_PREFIX = "cpp_";
const PYTHON_DIR = "db-python/trap/python";
const PYTHON_LIB_DIR = "db-python-lib/trap/python";
const CPP_DIR = "db-cpp/trap/cpp";
const MERGED_ROOT = process.argv[2] || "db-merged"
const MERGED_DIR = MERGED_ROOT + "/trap/cpython";
...
// python
console.log("Rewriting python trap..");
walk(PYTHON_DIR, ".trap.gz").forEach(from => {
  to = from.replace(PYTHON_DIR, MERGED_DIR);
  lines = readLines_gz(from);
  rewritten = lines.map(rewriteLine(PYTHON_PREFIX));
  createParentDir(to);
...
walk(CPP_DIR + '/tarballs','.br').forEach(tar_br => {
  ...
  try {
    ...
    fs.rmSync('__temp__', {recursive:true});
    return;
  }
