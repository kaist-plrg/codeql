...
import CppDataFlowImpl as CppDataFlowImpl

/**
 * A configuration for finding flow regarding interaction b/w java and cpp
 */
class JniConfiguration extends CppDataFlowImpl::Configuration {
...
JavaClassNode getJavaClassNode(ArgNode arg) {
...
JavaMethodNode getJavaMethodNode(ArgNode arg) {
  exists(JniConfiguration config, Node mid |
    (
...
JavaMethodNode getWrongJavaMethodNode(ArgNode arg) {
  exists(JniConfiguration config, Node mid |
    (
...
JavaFieldNode getJavaFieldNode(ArgNode arg) {
  exists(JniConfiguration config, Node mid |
    jniGetFieldIDStep(result, mid) and
    config.hasFlow(mid.asCppNode(), arg.asCppNode())
  )
}
JavaFieldNode getJavaStaticFieldNode(ArgNode arg) {
  exists(JniConfiguration config, Node mid |
...
module StringLiteralFlow {
  ...
  pragma[inline]
  string getStringLiteral(ArgNode arg) {
    exists (Node stringNode |
