...
class VariableParameterNode extends ParameterNode, UninitializedNode {
  VariableParameterNode() { this.getType().toString() = "va_list" }

  override predicate isParameterOf(Function fun, int i) {
    fun = this.getFunction() and
    i = fun.getNumberOfParameters() + [0, 1, 2]
  }
}
