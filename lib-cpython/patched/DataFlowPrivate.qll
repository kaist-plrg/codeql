...
class FunctionCall extends DataFlowCall, TFunctionCall {
  ...
  FunctionCall() {
    this = TFunctionCall(call)
    //and (not isStubFunction(callable)) implies (call = callable.getACall())
  }
  ...
  override DataFlowCallable getCallable() { /* result = callable */ call = result.getACall() }
