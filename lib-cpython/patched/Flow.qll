...
private AstNode toAst(ControlFlowNode n) { python_py_flow_bb_node(n, result, _, _) }
 ...
 */
class ControlFlowNode extends @python_py_flow_node {
  /** Whether this control flow node is a load (including those in augmented assignments) */
  predicate isLoad() {
    exists(Expr e | e = toAst(this) | python_py_expr_contexts(_, 3, e) and not augstore(_, this))
  }

  /** Whether this control flow node is a store (including those in augmented assignments) */
  predicate isStore() {
    exists(Expr e | e = toAst(this) | python_py_expr_contexts(_, 5, e) or augstore(_, this))
  }

  /** Whether this control flow node is a delete */
  predicate isDelete() { exists(Expr e | e = toAst(this) | python_py_expr_contexts(_, 2, e)) }

  /** Whether this control flow node is a parameter */
  predicate isParameter() { exists(Expr e | e = toAst(this) | python_py_expr_contexts(_, 4, e)) }
  ...
  ControlFlowNode getASuccessor() { python_py_successors(this, result) }

  /** Gets the immediate dominator of this flow node */
  ControlFlowNode getImmediateDominator() { python_py_idoms(this, result) }

  /** Gets the syntactic element corresponding to this flow node */
  AstNode getNode() { python_py_flow_bb_node(this, result, _, _) }
  ...
  predicate isEntryNode() { python_py_scope_flow(this, _, -1) }
  ...
  predicate isBranch() { python_py_true_successors(this, _) or python_py_false_successors(this, _) }
...
class NameNode extends ControlFlowNode {
  NameNode() {
    exists(Name n | python_py_flow_bb_node(this, n, _, _))
    or
    exists(PlaceHolder p | python_py_flow_bb_node(this, p, _, _))
  }
...
class NameConstantNode extends NameNode {
  NameConstantNode() { exists(NameConstant n | python_py_flow_bb_node(this, n, _, _)) }
...
private module Scopes {
  ...
  private predicate maybe_defined(SsaVariable var) {
    exists(var.getDefinition()) and not python_py_ssa_phi(var, _) and not var.getDefinition().isDelete()
    or
    exists(SsaVariable input | input = var.getAPhiInput() | maybe_defined(input))
  }

  private predicate maybe_undefined(SsaVariable var) {
    not exists(var.getDefinition()) and not python_py_ssa_phi(var, _)
    or
...
class BasicBlock extends @python_py_flow_node {
  BasicBlock() { python_py_flow_bb_node(_, _, this, _) }

  /** Whether this basic block contains the specified node */
  predicate contains(ControlFlowNode node) { python_py_flow_bb_node(node, _, this, _) }

  /** Gets the nth node in this basic block */
  ControlFlowNode getNode(int n) { python_py_flow_bb_node(result, _, this, n) }
  ...
  ControlFlowNode getLastNode() {
    exists(int i |
      ...
      i = max(int j | python_py_flow_bb_node(_, _, this, j))
    )
  }
  ...
  Scope getScope() {
    exists(ControlFlowNode n | n.getBasicBlock() = this |
      ...
      not python_py_scope_flow(n, _, -1) and
      not python_py_scope_flow(n, _, 0) and
      not python_py_scope_flow(n, _, 2) and
      result = n.getScope()
      or
      python_py_scope_flow(n, result, _)
    )
  }
