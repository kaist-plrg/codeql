...
that relate itself with other elements. For more information about QL, one can refer to
...
lifted classes and predicates without manual efforts.
...
and the predicate \codeql{viableCallableP2C}. \codeql{PyMethodDef} is the class
that corresponds to the C structure \ccode{PyMethodDef}. It has
...
\codeql{f = viablaCallableP2C(call)} holds when there exists a \codeql{PyMethodDef def}
such that \codeql{f = def.getFunc()} and
...
position does not work, and we need another way.
...
for representing the flow in and out of this node. First we define the store step
from the original argument node to the virtual argument node. Then we define
the step from the virtual argument node to the sole parameter node in C function.
By doing so, the flow form original argument node to the use site will be
found in three steps: 1) from the argumentnode to the virtual argument node,
2) from the virtual argument to the parameter node of C function, and 3) from the parameter to the out node of \ccode{PyArg_ParseTuple} API function call.
...
1) dataflows from Python function object to an argument of C function call,
and 2) dataflows from the parameters of C functions to the first argument of \ccode{PyObject_CallObject}. 
Using these two intra-flow analyses, we can find the actual target that should be invoked
for the API function call \ccode{PyObject_CallObject}.
