...
\rcode{ ln} denotes the line number and \rcode{ name} denotes a name of the function
...
example, we can extract the following syntactic facts: \rcode{ EnclosingStmt(1, 2,
0)}.
...
to \rcode{ z}, where \rcode{ x} and \rcode{ z} represents a program entity that can hold
...
\noindent
where \rcode{ Step(x, z)} denotes a direct data flow from \rcode{ x} to \rcode{ z}. The
