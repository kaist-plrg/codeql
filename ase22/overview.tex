...
are two functions, \rcode{m1} at line~3 and \rcode{m2} at line~5. The function \rcode{
m1} calls the function \rcode{f} in {\it B} via language interoperation between
{\it A} and {\it B}, and the function \rcode{m2} does nothing. At line~8 in {\it
A}, the expression \rcode{m1()} calls the function \rcode{m2} defined at line~3 to
invoke the function {\tt f} in {\it B}.
The \rcode{f} function in {\it B} calls the \rcode{CallMethod} function with a
...
about function call relations. In this example, a kind of facts, \rcode{
CallEdge(ln1, ln2)}, denotes a call relation from a callsite at line \rcode{ ln1}
to a function define at line \rcode{ ln2}. To derive function call relation facts,
...
make a query. Since the rule derives the \rcode{ CallEdge(7,3)} fact, the
following query produces \rcode{ (X, Y) $\in$ \{(7, 3)\}} as a result:
...
\noindent
The kind of facts \rcode{ ProcedureAt(ln, name)} is identical to \rcode{
FunctionAt(ln, name)} for the language {\it A}, but has a different relation
name. The kind of facts \rcode{ Argument(ln, i, arg)} denotes the syntactic
information of a function call argument, where \rcode{ ln} is the line number of
the callsite, \rcode{ i} is the argument position, and \rcode{ arg} is the argument
value.
...
different facts for different languages.  The query system produces \rcode{ (X, Y)
$\in$ \{\}} as a result of the query \rcode{ ?- CallEdge(X, Y)}, since there are
...
CallEdge$_{AB}$} and \rcode{ CallEdge$_{BA}$}. The kind of facts \rcode{
...
produces functions call relations as \rcode{ (X, Y) $\in$ \{(7, 3), (4, 9), (10,
