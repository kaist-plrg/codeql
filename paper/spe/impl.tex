...
\inblue{
As being a prototype implementation, \ours does not aim for a sound or complete
analyzer, yet it can effectively track dataflow in practical multilingual
programs using the simple approach explained in previous section.
}
...
One characteristic feature of QL is that it supports syntax of class similar to object-oriented
programming languages like Java. A QL class should be extended from another type,
...
Also, a QL class allows defining member predicates, as if defining the methods of class.
...
predicates.  For example, the characteristic predicate \codeql{OneOrTwo} can be
...
\end{lstlisting}
\inblue{
Then, every use site of the class can also be transformed to use the desugared predicates.
