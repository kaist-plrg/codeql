...
module system and name each of them \codeql{C} and \codeql{JAVA} so that we can distinguish the original
...
\inred{However, in practice, intra-language dataflow analysis is enough in most cases.}
...
\inred{Then, we make a rule that for \codeql{def} of type \codeql{PyMethodDef},
...
for representing flows in and out of this node. \inred{First, we define a
step to store the values of original argument nodes to virtual argument nodes. We then define
a step to store the values of virtual argument nodes to a single parameter node in a C function.
...
all arguments. 
\inred{Because C code can get Python objects only as
