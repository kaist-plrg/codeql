...
\end{lstlisting}
\noindent
An equality formula \codeql{m = addOne(n)} is also a syntactic sugar equivalent
...
\codeql{OneOrTwo}, because its characteristic predicate is satisfied only when
...
\codeql{add} is equivalent to the following ternary predicate:
\begin{lstlisting}[style=codeql,xleftmargin=2.5em]
