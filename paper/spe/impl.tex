...
\inblue{For example, Figure~\ref{fig:qll} (a) and (b) show sample QL libraries for C
...
\codeql{asJavaNode}, that downcast the elements of the lifted class into the elements
...
\inblue{Soundly answering these questions requires inter-language dataflow analysis
...
and the member field \ccode{ml_meth} has \inblue{the pointer to the actual C function.}
...
\inblue{Then, we make a rule for \codeql{viableCallableP2C} such that for some \codeql{def} of type \codeql{PyMethodDef},}
...
\inblue{step that represents the flow from values of the original argument nodes to virtual argument nodes. We then define
