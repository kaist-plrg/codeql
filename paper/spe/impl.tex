...
\inblue{As a prototype implementation, \ours does not aim for a sound or complete
...
\end{lstlisting}
\noindent
\inblue{Then, every use site of the original predicate is rewritten to
...
\end{lstlisting}
\inblue{Then, every use site of the class is rewritten to use the desugared predicates.
...
\inblue{In rare cases, this may result in missing some call edges}
...
analyze interactions \inblue{ between languages other than Java-C or Python-C
and even interactions} between more than two programming languages
\inblue{since providing inter-language FFI call relations enables CodeQL dataflow analysis.}
However, as Li et al.~\cite{vulnerable-multi} explain, \inblue{various interoperation mechanisms
...
\inblue{One possible approach could be dynamically collecting various kinds of language interactions,
