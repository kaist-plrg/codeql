...
one can \inblue{depict} rules by defining ``predicates'' and ``classes.'' The following
QL code defines the \inblue{unary} predicate \codeql{isOneOrTwo}:
...
\noindent
\inblue{which states that the fact \codeql{isOneOrTwo(n)} is derivable from either
...
\end{lstlisting}
\noindent
\inblue{The predicate is a syntactic sugar equivalent to the following ordinary predicate:}
%For example,
...
\end{lstlisting}
\noindent
\inblue{An equality formula \codeql{m = addOne(n)} is also a syntactic sugar equivalent
...
\end{lstlisting}
\noindent
\inblue{The class extends {\tt int}, which means that the class is a subtype of {\tt
int}.
...
\end{lstlisting}
\inblue{Thus, an equality formula \codeql{z = x.add(y)} is equivalent to
...
1) it stores the extracted information in a \inblue{\textit{trap}} file,
a human-readable \inblue{format file for the CodeQL database, and 2) it transforms the trap
file into a database in binary format.}
...
separate trap file for each language and then merges them \inblue{into a trap file.
One problem is that if each trap file has a table with the same name,
the merge fails due to the name collision.
To avoid the problem, \ours renames each table to a globally unique name by
appending a language-specific prefix to the table name before the merge.
...
For example, \inblue{Figure~\ref{fig:qll} (a) and (b)} show sample QL libraries for C
and Java\inblue{, respectively,} which use the same class name \ccode{Node} and the same
...
\inblue{\ours lifts} each library to the common level.
First, as on lines 1 and 2 of Figure~\ref{fig:qll3}, \inblue{\ours encapsulates} the
original dataflow libraries \inblue{for C and Java in separate modules named {\tt C}
and {\tt Java},
...
\inblue{\ours lifts} a class by first defining a sum
...
\inblue{The lifted class defines} two member predicates, \codeql{asCNode} and
\codeql{asJavaNode}, that \inblue{downcast the elements of the lifted class into the elements
of the original} C or Java class.
Lines 13 to 17 of Figure~\ref{fig:qll3} demonstrate the lifted predicate
\inblue{of the original predicate {\tt localFlowStep}}.
Similarly, \inblue{\ours lifts} a predicate by combining two original predicates with
the \codeql{or} connective. For each predicate, arguments and return values \inblue{are
downcasted to} the elements of the class of their corresponding language.
...
\inblue{\ours can} fully automate the lifting process using QL compiler error
...
signatures of the predicates. Using the information, \inblue{\ours
...
After lifting libraries for different languages, we \inblue{manually} extend predicates
...
by using \inblue{intra-language} analysis instead of \inblue{inter-language} analysis
as a trade-off for a more lightweight and simpler implementation.
We implemented two \inblue{intra-language flow} analysis modules for C,
...
we define two
\inblue{``intra-language flows''} analyses to identify Python objects assigned to \ccode{func}:
...
Using these \inblue{intra-language flow} analyses, we can find actual targets that should be invoked
