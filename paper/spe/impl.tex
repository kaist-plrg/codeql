...
\inred{However, using the final result of the inter-language dataflow analysis itself
...
Then, we make a rule for \codeql{viableCallableP2C} such that for \inred{some} \codeql{def} of type \codeql{PyMethodDef},
...
a step that represents \inred{the} flow from the values of virtual argument nodes to a single parameter node in a C function.
