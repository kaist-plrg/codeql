...
\noindent
\inblue{which states that the fact \codeql{isOneOrTwo(n)} is derivable from either one
...
\end{lstlisting}
\noindent
\inblue{The predicate is syntactic sugar equivalent to the following ordinary predicate:}
%For example,
...
\end{lstlisting}
\noindent
\inblue{An equality formula \codeql{m = addOne(n)} is also syntactic sugar equivalent
...
readable \inblue{format file for the CodeQL database, and 2) it transforms the trap
...
\inblue{step that represents the flow from values of original argument nodes to virtual argument nodes. We then define
