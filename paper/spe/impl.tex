...
one can \inred{represent} rules by defining ``predicates'' and ``classes.'' The following
...
\inred {
which states that the fact \codeql{isOneOrTwo(n)} can be derived from either
one of the two facts: \codeql{n = 1} or \codeql{n = 2}.
Some predicates may have results, as in the following predicate \codeql{addOne}:
}
\begin{lstlisting}[style=codeql,xleftmargin=2.5em]
int addOne(int n) {
  isOneOrTwo(n) and
...
\inred {
These kinds of predicates are syntactic sugars of the ordinary predicates. For example,
...
The following QL code defines the class \codeql{OneOrTwo}:
\begin{lstlisting}[style=codeql,xleftmargin=2.5em]
...
``characteristic predicate.'' \inred{For example, all possible values with type
\codeql{OneOrTwo} are 1 and 2, because these are the only values that satisfy the
characteristic predicate \codeql{OneOrTwo}}.
A class can also have ``member predicates''
that relate itself with other values.
\inred{
Fundamentally, member predicates are also
a syntactic sugar for ordinary predicates. For example, the member predicate \codeql{add} can be
rewritten as a ternary predicate \codeql{addPred}:
}
\begin{lstlisting}[style=codeql,xleftmargin=2.5em]
...
\end{lstlisting}
\inred{
and the formula \codeql{z = x.add(y)} is equivalent to \codeql{addPred(x, y, z)}.
}
...
facts from the source code.  Then, \inred{it merges} the common facts and rules in both
...
1) it stores the extracted information in a \inred{\textit{trap
file}}, a human readable file format \inred{for database}, and 2) it \inred{transforms} trap files
...
\inred{One problem is that if both trap files have tables with same name,
then the transformation fails due to the name collision.
To avoid this problem, \ours adds a language-specific prefix
to each table before the transformation.  
For example, if both trap files have tables named \codeql{@params},
the table from C is renamed as \codeql{@c\_params}, and the table from Java is renamed as
\codeql{@java\_params}.}
After renaming such tables, \inred{\ours can safely transform} the trap files into
...
classes and predicates by lifted ones. \inred{The original classes or predicates can later be referenced
using using the syntax \codeql{::}. For example, \codeql{Java::Node} refers to the original class
\codeql{Node} defined in the library for Java.}
...
\codeql{asCNode} and \codeql{asJavaNode},
...
the \codeql{or} connective. For each predicate, arguments and return values are casted down
...
After lifting libraries for different languages, we \inred{manually} extend predicates to
...
by using \inred{intra-language} analysis instead of \inred{inter-language} analysis
