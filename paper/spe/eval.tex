...
\inred{
For Java-C analyses, we use NativeFlowBench~\cite{nativeflowbench, JN-SAF} and
real world JNI Android apps downloaded from F-Droid~\cite{fdroid} as the benchmark suites.
For Python-C analyses, we use ExtModuleFlowBench and
...
\inred{By contrast}, because CodeQL under-approximates dataflows on arrays,
...
42 real-world Android apps \inred{downloaded from the F-Droid.}.
...
\inred{We chose to explore the same JNI interoperation bugs using \ours as those
...
\ours does not report any false positives for \inred{these three bugs, so we omitted
columns for the number of false positives of these bugs in the table.}
We confirmed the true and false positives by manual inspection of the source code.
\inred{We color the each cell for the number of false positives with non-zero value,
...
\inred{By contrast}, \lees fails to detect 12 out of 33 genuine bugs
...
detects from real-world apps. \inred{\sout{using simplified code}}

Figure~\ref{fig:bug1}\inred{is an excerpt from Graph 89 demonstrating} {\it NullDeref} bug.
...
Figure~\ref{fig:bug2} \inred{is an excerpt from ObscuraCam demonstrating} the {\it MissingFun} bug.
...
Figure~\ref{fig:bug3} \inred{is an excerpt from Document Viewer demonstrating} the {\it TypeMismatch} bug.
...
Figure~\ref{fig:bug4} \inred {is an excerpt from PrBoom demonstrating} the {\it WrongSig} bug.
