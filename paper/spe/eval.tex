...
This result indicates that \ours can
successfully perform dataflow analysis in the JNI programs with various JNI interoperations,
and its correctness is comparable with that of \jnsaf.
...
For this real-world benchmark, we compare analysis result of \ours with \lees~\cite{LeeASE20},
a state-of-art general purpose Java-C program analyzer.
We choose \lees as a comparison target instead of \jnsaf,
...
The first column shows app names with indices, the second shows lines of codes,
...
three million lines of code in about twelve minutes.

In summary, the result show that \ours can precisely analyze the real-world
applications that include JNI uses, in that more than 4 out of 5 JNI uses can
be resolved. In addition, the scalability of \ours is comparable with \lees, in that
it takes similar amount of time to analyze the smaller apps for both analyzers,
and \ours can analyze larger apps much faster than \lees.
...
In the failed benchmarks, C code changes the values of the fields in Python objects that are passed
to the parameter of C functions.
\ours does not handle such case because
the use of virtual argument node prevents the propagation of changed value
back to the field of original Python object, and it
reports false negatives for them.

This result show that our approach is feasible for performing
dataflow analysis not only in Java-C programs but also in Python-C programs,
yet there are some specific Python-C interoperation features that
\ours could not handle.
...
The first column shows repository names, the second shows lines of codes,
the thrid show database creation time,
...
This result indicate that \ours could perform dataflow analysis on
real world Python-C programs with high precision, and could
successfully find the python object values that C variables can have.
...
As a result, we can conclude that \ours is useful enough
to detect the various JNI interoperation bugs in the real-world JNI apps.
