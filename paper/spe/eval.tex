...
a ``dangerous'' \inblue{function}, such as \ccode{strncpy}, \ccode{strncmp},
...
{\it cvxopt} that \ours \inblue{ did} not detect.  The call to \ccode{strncmp} on line
...
\ours \inblue{ did} not detect this inter-use simply because the function
...
\textbf{\inblue{Extension}} \ours can analyze interoperations
between multiple languages via explicit Foreign Function Interfaces (FFIs)
\inblue{between two predefined sets of languages: Java-C and Python-C.
We suggest a several extensions that enables our analyzer to handle beyond this scope.
First, we can extend the analyzer to support interaction between various language sets.
}  As long as the analysis target languages use FFIs, our approach can
analyze interactions between them. \inblue{ This means that
we can easily extend \ours to support the multilingual program
which is written in languages other than Java-C or python-C,
and/or programs written in more than two programming languages.
Second, we can extend the analyzer to support interoperations other than FFIs.
We could easily handle FFIs since it easily fits with
the existing dataflow analysis framework of CodeQL; CodeQL's
dataflow module uses function call relations as a building block
for the dataflow analysis, so providing the inter-language FFI call relations
was enough to enable the dataflow analysis.}
However, as Li et al.~\cite{vulnerable-multi} explain, \inblue{there are various interoperation mechanisms
beyond explicit FFIs, such as implicit interaction between languages using interprocess communication
(e.g., socket-based message passing) or a language totally embodying another language
(e.g., web browser engines),} in real-world multi-language software systems.
Supporting these kinds of interoperation would be a promising future direction.
\inblue{
One way to do that would be to first design a framework consisting of
predicates that provides an abstract way to generalize all kinds of language interactions, not
limited to FFI calls (as if dataflow framework in CodeQL can be used in language agnostic way)
, and then instantiate the framework with the various language interactions.}
