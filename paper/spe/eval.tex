...
subset of targets of Li et al.~\cite{polycruise}'s work, which were compilable}.
...
In the remaining three programs, \ours resolves \inred{89 out of 123 (72.4\%)} total
API function calls. 
It fails to resolve \inred{34} API function calls due to implicit method calls in Python. 
...
program cvxopt that \ours could not detect.  The call to the \ccode{strncmp} at
