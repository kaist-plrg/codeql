...
\begin{itemize}
  ...
  \item \textbf{\req{2}: Usefulness.} \inblue{Is \ours useful for detecting} dataflow-related
...
collected from public GitHub repositories. \inblue{Six of them} are analysis
...
%to handle.
%
In the remaining three programs, \ours resolves \inblue{8,082 out of 11,725 (68.9\%)} total
API function calls. 
\inblue{The main for failing to resolve rest of API function calls was the implicit method calls in Python.}
...
\inblue{We evaluate the usefulness of \ours for detecting interoperation bugs in
...
\inblue{We omit the columns denoting false positivies for the three bugs in the table,
...
\inblue{Figure~\ref{fig:bug1} is an excerpt from the app, Graph 89, demonstrating {\it
NullDeref} bug.}
