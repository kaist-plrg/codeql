...
\[
  \begin{array}{llll}
    f & ::= & p(\overline{\inred{k}\ |\ x}) & \text{\sc Fact}\\
...
The fact $p(\overline{\inred{k}\ |\ x})$ denotes a relation $p$ between \inred{constant values}
$\overline{\inred{k}}$ \inred{such as string and integer values}, or variables $\overline{x}$. A rule $f'\
...
make a query. \inred{For example, the following query finds all the possible
assignment of the values to the variable \rcode{ X} and \rcode{ Y}.}
...
\noindent
\inred{
Since the rule derives the fact \rcode{ CallEdge(6,2)}, the
query system produces \rcode{ (X, Y)\ $\in$ \{(6, 2)\}} as a result of the query.
}
