...
\inred{\footnote{There is a syntactic restriction regarding negation and recursive rules:
A fact should not be derived from a negation of it. For example, the rule $f(x) :-\ \neg f(x)$
is not a syntactically valid rule, since $f$ is derived from the negation of itself.}}
