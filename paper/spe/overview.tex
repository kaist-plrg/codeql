...
$\overline{x}$.  A fact A rule $f'\ \mbox{:-}\ \overline{f\ |\neg f}$ consists of two
...
known facts. \inblue{ Note that the derived facts should not contain
free variable as the argument.}
...
Finally, \inblue{we can make the exact same query} \rcode{ ?- CallEdge(X, Y)} \inblue{as we did for both of} the
monolingual programs in Section~\ref{lab:ovmono}. \inblue{Note that this query is valid
for the multilingual program as well, since we specifically defined the new rule to derive the fact having
