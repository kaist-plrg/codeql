...
An example declarative static analysis consists of the following \inred{datalog-like} facts, rules, and queries:
...
\noindent
\inred{We use the overline notation for indicating the 0 or more repetitions. For example,
$\overline{x}$ is a shorthand for $x_1, x_2, ..., x_n$ for some $n \ge 0$. We use the }
The fact $p(\overline{\inred{v}\ |\ x})$ denotes a relation $p$ between \inred{primitive values}
$\overline{\inred{v}}$ \inred{such as string and integer values}, or variables $\overline{x}$. A rule $f'\
\mbox{:-}\ \overline{\inred{f\ |\neg f}}$ consists of two parts: the head of the rule $f'$
and the body of the rule \inred{$\overline{f\ |\neg f}$, where the optional prefix $\neg$ denotes
the negation.\footnote{One restriction regarding negation is that there should not be
any recursive rules with odd number of negations.}}
It denotes that the head can be derived from the body:
$f'$ \inred{can be derived if all the facts $f'' \in \{\overline{f}\}$ are included in the set of
known facts, and the all the facts $f'' \in \{\overline{\neg f}\}$ are not included in the set of known facts}.
The body of the rule can be empty, and in that case, the head of the rule can be vacuously added to the known facts.
Such bodiless rules can be considered as the initial input facts for the analysis.
The query $\mbox{?-}\ \overline{f}$ finds all possible variable assignments that
make all facts in $\overline{f}$ \inred{to be included in the set of known facts} if variables in $\overline{f}$ are
replaced with the assigned \inred{values}.
...
string value \rcode{"m2"} to invoke the function \rcode{m2} in {\it A} code, using the
...
CallEdge(lineNum1, lineNum2)}, denotes a call relation from a callsite on line \rcode{ lineNum1}
to a function definition on line \rcode{ lineNum2}. To derive function call relation facts,
...
\noindent
The rule derives a new fact \rcode{ CallEdge(lineNum1, lineNum2)} when a callsite on line
\rcode{ lineNum1} has the same target function name as the name of a function defined
on line \rcode{ lineNum2}. Therefore, a new fact can be derived from the initial facts
as follows: 
...
Finally, when making the same query \rcode{ ?- CallEdge(X, Y)}
\inred{as we did for the monolingual programs,}
the query system
now produces \rcode{ (X, Y) $\in$ \{(6, 2), (3, 7), } \rcode{(8, 4)\}} as function call relation results,
