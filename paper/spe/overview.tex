...
An example declarative static analysis consists of the following \inblue{Datalog-like} facts, rules,
and queries:

\[
  \begin{array}{llll}
    f & ::= & p(\overline{\inblue{k}\ |\ x}) & \text{\sc Fact}\\
    r & ::= & f'\ \mbox{:-}\ \overline{\inblue{f\ |\neg f}} & \text{\sc Rule}\\
    q & ::= & \mbox{?-}\ \overline{f}\ &  \text{\sc Query}
\end{array}
\]

\noindent
\inblue{We use the overline notation to indicate 0 or more repetitions. 
...
\footnote{A syntactic restriction exists for negation and recursive
...
facts.
The body of the rule can be empty; in that case, the head of the rule is
vacuously added to known facts.
Such bodiless rules are the initial input facts for the
analysis.
...
string \inblue{literal} \rcode{"m2"} to invoke the function \rcode{m2} in {\it A} code, using the
string \inblue{literal} argument as the target function name.
...
make a query. 
\inblue{For example, the following query finds all possible literals for the variables
\rcode{ X} and \rcode{ Y}:}
...
\end{lstlisting}
\noindent
\inblue{Since only one \rcode{ CallEdge} fact, \rcode{ CallEdge(6,2)}, belongs to known
...
\inblue{Finally, when making the query \rcode{ ?- CallEdge(X, Y)} as the same for the
