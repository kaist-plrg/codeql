...
and the body of the rule $\overline{\neg^? f}$.  It denotes that the head can be derived from the body:
...
hold. The body of the rule can be empty, and in that case, the head of the rule vacuously holds.
...
replaced with the assigned elements.
...
CallEdge(ln1, ln2)}, denotes a call relation from a callsite on line \rcode{ ln1}
to a function definition on line \rcode{ ln2}. To derive function call relation facts,
...
\noindent
The rule derives a new fact \rcode{ CallEdge(ln1, ln2)} when a callsite on line
\rcode{ ln1} has the same target function name as the name of a function defined
on line \rcode{ ln2}. Therefore, a new fact can be derived from the initial facts
as follows: 
...
Finally, when making the same query \rcode{ ?- CallEdge(X, Y)}, the query system
