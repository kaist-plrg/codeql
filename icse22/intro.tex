...
in multilingual programs, they often \inred{fail to analyze within a limited time budget}.
...
shows \inred{better scalability} without analysis precision degradation.
...
Using these bug detectors, we found \inred{25 new bugs and vulnerabilities from
real-world applications, among them 12 bugs were from the apps that \lees
failed due to a timeout or out of memory}.
...
\item We show the practical usefulness of the JNI program analyzer for real-world
  JNI programs. Our tool \inred{analyzed more apps within a timeout than the
